...
function appendReviewedSurchargeCostPreview(row, tableReview, candidate, surchargeReview, additionalCostEvidence = [], costScopeReviews = [], unsupportedCostReviews = [], fxEvidence = []) {
  ...
  const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label,cargoLabel,routingLabel,viaLabel,referenceDate.label,evidenceInquiry.label,scopeReviewLabel,unsupportedReviewLabel,additionalLabel,fxReferenceAt.label,fxLabel,shipmentCount.label,awbCount.label,hawbCount.label,mawbCount.label);box.append(grid,node("span","Additional/local cost kayıtları otomatik seçilmez. Seçim exact inquiry/source ile eşleşir ve quantity basis için matching count ister. Cross-currency surcharge FX yoksa currency_mismatch_no_fx olarak hariç kalır; seçilmiş cross-currency local cost ise matching FX yoksa fail-closed olur. Inverse/nearest/latest kur yoktur. Tarife referans tarihi boşsa bugünün tarihi varsayılmaz. Count alanları boş bırakılırsa hiçbir count varsayılmaz. Bu subtotal hâlâ all-in değildir.","small muted"),run,result,coverageBox,completenessBox,readinessBox);row.append(box);
}
...
function renderAirRateSourceSettings(payload = {}, validityPayload = {}, roundingPayload = {}, additionalCostPayload = {}, costScopePayload = {}, unsupportedCostPayload = {}, fxPayload = {}, availabilityPayload = {}, reviewsPayload = {}, tablePayload = {}, surchargePayload = {}) {
  const panel=node("section","","settings-panel"); const h=node("div","","settings-heading");
