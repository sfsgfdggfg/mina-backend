...
function appendReviewedSurchargeCostPreview(row, tableReview, candidate, surchargeReview, fxEvidence = []) {
  if(!surchargeReview || surchargeReview.status!=="completed") return;
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Reviewed Surcharge Cost Preview"),node("span","Human-review evidence ile partial reference cost. Cross-currency kalem yalnız explicit seçilmiş aynı inquiry/source/direction/timestamp FX evidence ile çevrilir; ters veya otomatik FX yok. All-in değildir · Müşteri teklifi değildir · Capacity/booking teyidi değildir.","small muted"));
  ...
  const fxInquiry=inboxField("FX inquiry reference");
  ...
  const run=actionButton("Reviewed Surcharge Cost Hesapla","",async()=>{run.disabled=true;result.textContent="Hesaplanıyor…";try{const body={actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;if(referenceDate.input.value)body.reference_date=referenceDate.input.value;const selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);if(selectedFx.length){const inquiry=fxInquiry.input.value.trim(),ts=fxReferenceAt.input.value.trim();if(!inquiry||!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde inquiry reference ve timezone-offset'li FX reference timestamp gerekli.");body.inquiry_reference=inquiry;body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/reviewed-surcharge-cost-preview`,{method:"POST",body:JSON.stringify(body)});const perKg=(out.included_surcharges||[]).map(x=>x.fx_evidence_id?`${x.surcharge_code}: ${x.source_surcharge_cost} ${x.source_currency} × FX ${x.fx_rate} = ${x.surcharge_cost} ${x.currency} (${x.application_basis}; ${x.fx_evidence_id})`:`${x.surcharge_code}: ${x.applied_weight_kg} kg × ${x.rate_per_kg} = ${x.surcharge_cost} ${x.currency} (${x.application_basis})`).join(" | ")||"Dahil edilen per-kg surcharge yok";const flat=(out.included_flat_surcharges||[]).map(x=>x.fx_evidence_id?`${x.surcharge_code}: ${x.source_surcharge_cost} ${x.source_currency} × FX ${x.fx_rate} = ${x.surcharge_cost} ${x.currency} (${x.quantity_basis}; ${x.fx_evidence_id})`:`${x.surcharge_code}: ${x.applied_count} × ${x.amount_per_unit} = ${x.surcharge_cost} ${x.currency} (${x.quantity_basis})`).join(" | ")||"Dahil edilen flat surcharge yok";const excluded=(out.excluded_surcharges||[]).map(x=>`${x.surcharge_code}/${x.currency}: ${x.reason}`).join(" | ")||"Hariç kalem yok";const rounding=out.freight.rounding_review_id?(out.freight.rounding_mode==="none"?"reviewed: none":`reviewed ceiling ${out.freight.rounding_increment_kg} kg → ${out.freight.rounded_chargeable_weight_kg} kg`):"unreviewed / raw chargeable";result.textContent=`Base freight ${out.freight.recommended_base_freight} ${out.freight.currency} · Weight rounding ${rounding} · Reviewed per-kg ${out.reviewed_per_kg_surcharge_total} · Reviewed flat ${out.reviewed_flat_surcharge_total} · Base + reviewed surcharge ${out.base_plus_reviewed_surcharges} ${out.freight.currency} · Tarife geçerliliği ${out.tariff_validity_confirmed?`DOĞRULANDI (${out.reviewed_valid_from} → ${out.reviewed_valid_to}; ref ${out.reference_date})`:"DOĞRULANMADI"} · FX ${out.fx_applied?`UYGULANDI (${(out.fx_evidence_ids_used||[]).join(", ")})`:"YOK"} · ALL-IN DEĞİL · Per-kg: ${perKg} · Flat: ${flat} · Hariç: ${excluded}`;}catch(e){result.textContent=e.message||String(e);}finally{run.disabled=false;}});
  const readinessResult=node("div","","small muted");
  const readiness=actionButton("Operational Readiness Kontrol Et","",async()=>{readiness.disabled=true;readinessResult.textContent="Kontrol ediliyor…";try{const inquiry=fxInquiry.input.value.trim();if(!referenceDate.input.value||!inquiry)throw new Error("Operational readiness için service date ve inquiry reference gerekli.");const body={inquiry_reference:inquiry,service_date:referenceDate.input.value,actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;const selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);const ts=fxReferenceAt.input.value.trim();if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/operational-readiness-preview`,{method:"POST",body:JSON.stringify(body)});const blockers=(out.operational_blockers||[]).join(", ")||"yok";const rounding=out.cost_preview.freight.rounding_review_id?(out.cost_preview.freight.rounding_mode==="none"?"reviewed: none":`reviewed ceiling ${out.cost_preview.freight.rounding_increment_kg} kg → ${out.cost_preview.freight.rounded_chargeable_weight_kg} kg`):"unreviewed / raw chargeable";readinessResult.textContent=`Operational evidence ${out.operational_evidence_complete?"COMPLETE":"INCOMPLETE"} · Tarife ${out.tariff_validity_confirmed?"OK":"EKSİK"} · Capacity ${out.capacity_confirmed?"OK":"EKSİK"} · Schedule ${out.schedule_confirmed?"OK":"EKSİK"} · Weight rounding ${rounding} · FX ${out.fx_applied?`UYGULANDI (${(out.fx_evidence_ids_used||[]).join(", ")})`:"YOK"} · Blockers: ${blockers} · Partial cost ${out.cost_preview.base_plus_reviewed_surcharges} ${out.cost_preview.freight.currency} · Müşteri teklifi hazır değil · Booking authority yok`; }catch(e){readinessResult.textContent=e.message||String(e);}finally{readiness.disabled=false;}});
  const readinessBox=node("div","","supplier-commercial");readinessBox.append(node("strong","Operational Readiness Preview"),node("span","Exact inquiry + service date + source-bound tariff validity + capacity/schedule evidence tek görünümde kontrol edilir. Partial cost dışında customer quote veya booking authority yaratmaz.","small muted"),readiness,readinessResult);
  const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label,cargoLabel,routingLabel,viaLabel,referenceDate.label,fxInquiry.label,fxReferenceAt.label,fxLabel,shipmentCount.label,awbCount.label,hawbCount.label,mawbCount.label);box.append(grid,node("span","FX seçimi boşsa cross-currency kalemler currency_mismatch_no_fx olarak hariç kalır. FX evidence seçilirse inquiry + exact source/direction + timestamp birebir eşleşir; inverse/nearest/latest kur türetilmez. Tarife referans tarihi boşsa bugünün tarihi varsayılmaz. Count alanları boş bırakılırsa hiçbir count varsayılmaz.","small muted"),run,result,readinessBox);row.append(box);
}
