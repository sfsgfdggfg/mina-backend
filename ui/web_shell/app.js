...
function appendReviewedSurchargeCostPreview(row, tableReview, candidate, surchargeReview) {
  ...
  box.append(node("strong","Reviewed Per-Kg Surcharge Cost Preview"),node("span","Yalnız aynı currency + tüm human-review zinciri tamamlanmış per-kg surcharge'lar. Flat ve FX hariç · All-in değildir · Müşteri teklifi değildir · Tarife geçerlilik/capacity teyidi değildir.","small muted"));
  ...
  const run=actionButton("Reviewed Surcharge Cost Hesapla","",async()=>{run.disabled=true;result.textContent="Hesaplanıyor…";try{const body={actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/reviewed-surcharge-cost-preview`,{method:"POST",body:JSON.stringify(body)});const included=(out.included_surcharges||[]).map(x=>`${x.surcharge_code}: ${x.applied_weight_kg} kg × ${x.rate_per_kg} = ${x.surcharge_cost} ${x.currency} (${x.application_basis})`).join(" | ")||"Dahil edilen per-kg surcharge yok";const excluded=(out.excluded_surcharges||[]).map(x=>`${x.surcharge_code}/${x.currency}: ${x.reason}`).join(" | ")||"Hariç kalem yok";result.textContent=`Base freight ${out.freight.recommended_base_freight} ${out.freight.currency} · Reviewed per-kg surcharge ${out.reviewed_per_kg_surcharge_total} ${out.freight.currency} · Base + reviewed per-kg ${out.base_plus_reviewed_per_kg_surcharges} ${out.freight.currency} · ALL-IN DEĞİL · ${included} · Hariç: ${excluded}`;}catch(e){result.textContent=e.message||String(e);}finally{run.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label,cargoLabel,routingLabel,viaLabel);box.append(grid,run,result);row.append(box);
}
