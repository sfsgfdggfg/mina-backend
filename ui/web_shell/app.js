...
function renderAirRateSourceSettings(payload = {}, reviewsPayload = {}) {
  const panel=node("section","","settings-panel"); const h=node("div","","settings-heading");
  h.append(node("h2","Havayolu Listeleri"),node("p","Ticari havayolu tarife PDF'lerini shadow-learning için kaydeder ve yerelde yalnız yapısal etiketleri çıkarır. Fiyat satırlarını yorumlamaz, quote üretmez ve express tarifesi kabul etmez.","muted")); panel.append(h);
  panel.append(node("div","PDF ticari sır olarak korunur. Bu ekran fiyat üretmez. Yapı çıkarımı yerel/deterministik çalışır; AI parser çağrılmaz ve tam extracted text state/audit kaydına yazılmaz.","notice"));
  const sources=(payload.rate_sources||[]).slice(); const reviews=(reviewsPayload.reviews||[]).slice(); const reviewBySource=Object.fromEntries(reviews.map(r=>[r.source_id,r])); const list=node("div","","supplier-price-list");
  ...
  sources.forEach(item=>{const review=reviewBySource[item.source_id];const card=node("div","","supplier-commercial supplier-price-card");const validity=(item.valid_from||item.valid_to)?`${item.valid_from||"?"} → ${item.valid_to||"?"}`:"Geçerlilik belirtilmedi";card.append(node("strong",`${item.airline_name} · ${item.document_name}`),node("span",item.document_stored?"PDF saklandı":"PDF eksik",`badge ${item.document_stored?"open":""}`),node("span",`${scopeLabel[item.cargo_scope]||item.cargo_scope} · Çıkış ${item.origin_airport||"genel"}`,"small"),node("span",validity,"small"),node("span",`Yüklendi: ${formatDate(item.recorded_at)} · ${item.recorded_by}`,"small muted"));if(review){card.append(node("span",`${reviewLabel[review.status]||review.status} · ${review.candidates?.length||0} aday`,`badge ${review.status==="completed"?"open":""}`));}else if(item.document_stored){const extract=actionButton("Yapıyı Çıkar","",async()=>{extract.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/extract-structure`,{method:"POST"});await loadSettings();}catch(e){showError(e);extract.disabled=false;}});card.append(extract);}list.append(card);});
...
function renderSettings(branding, automationPolicy, customersPayload = {}, suppliersPayload = {}, performanceSettings = {}, relationshipStatus = {}, fixedRates = {}, airRates = {}, airRateReviews = {}, systemHealth = {}, customerMemoryBundle = null) {
...
async function loadSettings() {
  const [branding, automationPolicy, customersPayload, suppliersPayload, performanceSettings, relationshipStatus, fixedRates, airRates, airRateReviews, runtime, automation, dataHealth, commodityValidation, supplierValidation, customerMemoryValidation, hsValidation] = await Promise.all([
    api("/settings/branding"), api("/automation-policy/agency"), api("/master-data/customers"), api("/master-data/suppliers"), api("/settings/performance"), api("/relationship-onboarding/status"), api("/supplier-fixed-rates"), api("/air-rate-sources"), api("/air-rate-structure-reviews"), api("/runtime/release"), api("/automation/status"), api("/data-health/summary"), api("/commodity-dictionary/validation"), api("/supplier-capabilities/validation"), api("/customer-memory/validation"), api("/hs-commodity-map/validation")
  ]);
