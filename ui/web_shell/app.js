...
function renderAirRateSourceSettings(payload = {}) {
  const panel=node("section","","settings-panel"); const h=node("div","","settings-heading");
  h.append(node("h2","Havayolu Listeleri"),node("p","Ticari havayolu tarife PDF'lerini shadow-learning için kaydeder. Bu ekran fiyat üretmez, listeyi yorumlamaz ve express tarifesi kabul etmez.","muted")); panel.append(h);
  panel.append(node("div","PDF ticari sır olarak korunur; browser yalnız dosyayı yükler, uygulama ekranı fiyat içeriğini göstermez.","notice"));
  const sources=(payload.rate_sources||[]).slice(); const list=node("div","","supplier-price-list");
  const scopeLabel={general_cargo:"General cargo",special_cargo:"Özel kargo",mixed:"Karma",unknown:"Belirsiz"};
  sources.forEach(item=>{const card=node("div","","supplier-commercial supplier-price-card");const validity=(item.valid_from||item.valid_to)?`${item.valid_from||"?"} → ${item.valid_to||"?"}`:"Geçerlilik belirtilmedi";card.append(node("strong",`${item.airline_name} · ${item.document_name}`),node("span",item.document_stored?"PDF saklandı":"PDF eksik",`badge ${item.document_stored?"open":""}`),node("span",`${scopeLabel[item.cargo_scope]||item.cargo_scope} · Çıkış ${item.origin_airport||"genel"}`,"small"),node("span",validity,"small"),node("span",`Yüklendi: ${formatDate(item.recorded_at)} · ${item.recorded_by}`,"small muted"));list.append(card);});
...
function renderSettings(branding, automationPolicy, customersPayload = {}, suppliersPayload = {}, performanceSettings = {}, relationshipStatus = {}, fixedRates = {}, airRates = {}, systemHealth = {}, customerMemoryBundle = null) {
...
async function loadSettings() {
  const [branding, automationPolicy, customersPayload, suppliersPayload, performanceSettings, relationshipStatus, fixedRates, airRates, runtime, automation, dataHealth, commodityValidation, supplierValidation, customerMemoryValidation, hsValidation] = await Promise.all([
    api("/settings/branding"), api("/automation-policy/agency"), api("/master-data/customers"), api("/master-data/suppliers"), api("/settings/performance"), api("/relationship-onboarding/status"), api("/supplier-fixed-rates"), api("/air-rate-sources"), api("/runtime/release"), api("/automation/status"), api("/data-health/summary"), api("/commodity-dictionary/validation"), api("/supplier-capabilities/validation"), api("/customer-memory/validation"), api("/hs-commodity-map/validation")
  ]);
