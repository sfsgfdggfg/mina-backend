...
function appendAirAvailabilityEvidence(card, item, confirmations = []) {
  ...
  confirmations.slice(0,5).forEach(x=>box.append(node("span",`${x.inquiry_reference} · ${x.destination_code} · ${x.service_date} · capacity ${x.capacity_status} · schedule ${x.schedule_status}${x.flight_reference?` · ${x.flight_reference}`:""} · ${x.evidence_channel}/${x.evidence_reference}`,"small muted")));
  ...
  const via=inboxField("Via airport (opsiyonel)");via.input.maxLength=3;const serviceDate=inboxField("Service date","date");
  ...
  const save=actionButton("Kapasite / Schedule Teyidi Kaydet","",async()=>{const dest=destination.input.value.trim().toUpperCase();const viaCode=via.input.value.trim().toUpperCase();if(!inquiry.input.value.trim()||!/^[A-Z]{3}$/.test(dest)||!serviceDate.input.value||!evidenceRef.input.value.trim()||!note.input.value.trim()){fb.textContent="Inquiry, destination, service date, evidence reference ve note gerekli.";return;}if(routing.value==="direct"&&viaCode){fb.textContent="Direct routing via airport taşıyamaz.";return;}if(viaCode&&!/^[A-Z]{3}$/.test(viaCode)){fb.textContent="Via airport 3 harfli IATA kodu olmalı.";return;}if(schedule.value==="not_confirmed"&&flight.input.value.trim()){fb.textContent="Schedule not confirmed iken flight reference girilemez.";return;}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/availability-confirmations`,{method:"POST",body:JSON.stringify({entry_id:freshPriceEntryId("web-air-availability"),inquiry_reference:inquiry.input.value.trim(),destination_code:dest,routing_context:routing.value,via_airport:viaCode||null,service_date:serviceDate.input.value,capacity_status:capacity.value,schedule_status:schedule.value,flight_reference:flight.input.value.trim()||null,evidence_channel:channel.value,evidence_reference:evidenceRef.input.value.trim(),evidence_note:note.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(inquiry.label,destination.label,routingLabel,via.label,serviceDate.label,capacityLabel,scheduleLabel,flight.label,channelLabel,evidenceRef.label,note.label);box.append(grid,save,fb);card.append(box);
}
