...
function renderSupplierPricesSection(container, data, jobId, refresh) {
  ...
  if (data.job?.job_kind === "approved_job" && data.job?.stage === "pricing") {
    const award=data.supplier_award||{};const selected=award.current;
    const box=node("div","","approval-focused supplier-award-box");
    box.append(node("h3","Onaylı İş · Tedarikçi Seçimi"),node("div","Müşteri teklif adımları atlanır; operasyon açılmadan önce güncel bir tedarikçi fiyatı açıkça seçilmelidir.","small muted"));
    if(selected)box.append(node("div",`${selected.supplier_name} · ${moneyLabel(selected.cost,selected.currency)} · ${supplierPriceSourceLabel(selected.source_type)}${award.current_selection_usable?" · güncel":" · yeniden seçim gerekli"}`,award.current_selection_usable?"notice":"warning"));
    if(offers.length){const label=node("label","Tedarikçi fiyatı");const select=document.createElement("select");offers.slice().sort((a,b)=>new Date(b.recorded_at||0)-new Date(a.recorded_at||0)).forEach(item=>{const o=document.createElement("option");o.value=item.offer_id;o.textContent=`${item.supplier_name} · ${moneyLabel(item.cost,item.currency)} · ${supplierPriceSourceLabel(item.source_type)}`;select.append(o);});if(selected&&offers.some(item=>item.offer_id===selected.offer_id))select.value=selected.offer_id;label.append(select);const fb=node("div","","muted settings-feedback");const choose=actionButton(selected?"Seçimi Yenile":"Tedarikçiyi Seç","primary",async()=>{choose.disabled=true;try{await api(`/mina-jobs/${encodeURIComponent(jobId)}/supplier-award`,{method:"POST",body:JSON.stringify({offer_id:select.value})});await refresh();}catch(e){fb.textContent=e.message||String(e);choose.disabled=false;}});box.append(label,choose,fb);}
    if(award.current_selection_usable){const open=actionButton("Tedarikçiyi Seç ve Operasyonu Aç","approve",async()=>{open.disabled=true;try{await api(`/mina-jobs/${encodeURIComponent(jobId)}/stage`,{method:"POST",body:JSON.stringify({target_stage:"operation_opened"})});await refresh();}catch(e){box.append(node("div",e.message||String(e),"error"));open.disabled=false;}});box.append(open);}
    section.append(box);
  }
...
async function renderQuoteSection(container, data, refresh) {
  ...
  section.append(grid);
  const airContext = approval?.air_quote_context_snapshot || quoteCase.air_quote_context || null;
  if (airContext) {
    const airBox=node("div","","approval-focused customer-commercial-context");
    airBox.append(node("h3","Air Readiness Provenance · Frozen Snapshot"),node("div","Bu snapshot teklif hazırlanırken geçen tariff/cost/availability evidence zincirini dondurur; revizyonlarda aynı evidence lineage korunur.","muted small"));
    const airMetrics=node("div","","detail-grid");airMetrics.append(summaryItem("Havayolu",airContext.airline_name||"-"),summaryItem("Rota",`${airContext.origin_airport||"?"} → ${airContext.destination_code||"?"}`),summaryItem("Servis",airContext.service_date||"-"),summaryItem("Beklenen teslim",airContext.expected_delivery_date||"-"),summaryItem("Cost basis",moneyLabel(airContext.confirmed_cost_basis_amount,airContext.confirmed_cost_basis_currency)),summaryItem("Pricing source",codeLabel(airContext.pricing_policy_source||"-")),summaryItem("Availability evidence",airContext.availability_confirmation_id||"-"),summaryItem("Tarife satırı",airContext.candidate_id||"-"));airBox.append(airMetrics,node("div","Snapshot quote-send veya booking authority değildir; insan onayı ve mevcut send safety ayrı çalışır.","notice small"));section.append(airBox);
  }
...
function renderAirLearningSection(container, data, jobId, refresh) {
  if (data.summary?.transport_mode !== "air") return;
  const view = data.air_learning || {};
  const handoff = data.air_operation_handoff || null;
  if (!handoff && !(view.history || []).length) return;
  const section = sectionBlock("Havayolu Öğrenme Geri Beslemesi", "Gerçek outcome/correction evidence route-level öğrenme önerilerine dönüşebilir. Human-confirmed facts bile P2-44'te yalnız advisory'dir; fiyat, tarife, rota, booking veya outbound authority oluşturmaz.");
  const current = view.current || null;
  const ctx = handoff?.air_quote_context || current?.quoted_air_context || {};
  const advisory = view.advisory || {};
  const confirmed = advisory.confirmed_facts || [];
  if (advisory.subject_label) {
    const adv=node("div","","supplier-commercial");
    adv.append(node("strong",`Route Advisory · ${advisory.subject_label}`));
    if(confirmed.length) confirmed.forEach(f=>adv.append(node("div",`${codeLabel(f.fact_key)}: ${learningFactValueLabel(f.value,f.value_unit)} · güven %${Math.round((Number(f.confidence)||0)*100)} · Fact ${f.fact_id}`,"small policy-evidence")));
    else adv.append(node("div","Bu route için human-confirmed air learning henüz yok.","small muted"));
    adv.append(node("div","ADVISORY ONLY · pricing/tariff/routing/booking authority yok", "notice")); section.append(adv);
  }
  if (view.recordable) {
    const form=node("div","","settings-inline-editor approval-focused");
    form.append(node("strong",current?"Air outcome feedback düzeltmesi":"Gerçekleşen Air Outcome / Correction Evidence"));
    const usageLabel=node("label","Quoted tariff sonucu");const usage=document.createElement("select");[["used_as_quoted","Aynen kullanıldı"],["used_with_correction","Düzeltmeyle kullanıldı"],["not_used","Kullanılmadı"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;usage.append(o);});usage.value=current?.tariff_usage||"used_as_quoted";usageLabel.append(usage);
    const airline=inboxField("Gerçekte kullanılan havayolu");airline.input.value=current?.actual_airline_name||ctx.airline_name||"";
    const routingLabel=node("label","Gerçek routing");const routing=document.createElement("select");[["direct","Direct"],["connecting","Connecting"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;routing.append(o);});routing.value=current?.actual_routing_context||ctx.routing_context||"direct";routingLabel.append(routing);
    const via=inboxField("Gerçek via airport");via.input.maxLength=3;via.input.value=current?.actual_via_airport||ctx.via_airport||"";
    const service=inboxField("Gerçek service/departure date","date");service.input.value=current?.actual_service_date||"";
    const delivery=inboxField("Gerçek delivery date","date");delivery.input.value=current?.actual_delivery_date||"";
    const chargeable=numberField("Gerçek chargeable weight kg",current?.actual_chargeable_weight_kg??null,0.01,1000000);chargeable.input.step="0.01";
    const cost=numberField("Gerçek toplam air cost",current?.actual_cost_amount??null,0.01,1000000000);cost.input.step="0.01";
    const currency=inboxField("Gerçek cost currency");currency.input.maxLength=3;currency.input.value=current?.actual_cost_currency||ctx.confirmed_cost_basis_currency||"";
    const sourceLabel=node("label","Evidence source");const source=document.createElement("select");[["airline_invoice","Airline invoice"],["airline_booking_confirmation","Booking confirmation"],["airline_email","Airline e-posta"],["portal","Portal"],["operator","Operatör"],["other","Diğer"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;source.append(o);});source.value=current?.evidence_source||"airline_invoice";sourceLabel.append(source);
    const ref=inboxField("Evidence reference");ref.input.value="";ref.input.placeholder=current?.source_reference||"Invoice / booking / email referansı";
    const noteLabel=node("label","Outcome / correction notu");const note=document.createElement("textarea");note.rows=3;note.maxLength=1200;note.placeholder=current?.note||"Gerçekte ne oldu, quoted varsayımdan ne değişti?";noteLabel.append(note);
    const corrections=node("fieldset","","channel-fieldset");corrections.append(node("legend","Correction kategorileri"));const correctionChecks={};[["chargeable_weight","Chargeable weight"],["base_rate","Base rate"],["surcharge","Surcharge"],["local_cost","Local cost"],["fx","FX"],["airline","Airline"],["routing","Routing"],["service_date","Service date"],["delivery_date","Delivery date"],["other","Diğer"]].forEach(([v,t])=>{const l=node("label","","check-label");const i=document.createElement("input");i.type="checkbox";i.checked=(current?.correction_categories||[]).includes(v);correctionChecks[v]=i;l.append(i,node("span",t));corrections.append(l);});
    usage.addEventListener("change",()=>{if(usage.value==="used_as_quoted")Object.values(correctionChecks).forEach(i=>i.checked=false);});
    const grid=node("div","","settings-two-col");grid.append(usageLabel,airline.label,routingLabel,via.label,service.label,delivery.label,chargeable.label,cost.label,currency.label,sourceLabel,ref.label);form.append(grid,corrections,noteLabel);
    const feedback=node("div","","muted settings-feedback");
    const save=actionButton(current?"Düzeltme Evidence'ını Kaydet":"Air Outcome Feedback Kaydet","primary",async()=>{const correctionsSelected=Object.entries(correctionChecks).filter(([,i])=>i.checked).map(([k])=>k);const curr=currency.input.value.trim().toUpperCase();if(!airline.input.value.trim()||!ref.input.value.trim()||note.value.trim().length<3){feedback.textContent="Havayolu, evidence reference ve açıklama gerekli.";return;}if(routing.value==="connecting"&&!/^[A-Za-z]{3}$/.test(via.input.value.trim())){feedback.textContent="Connecting routing için 3 harfli via airport gerekli.";return;}if(routing.value==="direct"&&via.input.value.trim()){feedback.textContent="Direct routing via airport taşıyamaz.";return;}if((cost.value()==null)!=(curr.length===0)){feedback.textContent="Gerçek cost amount ve 3 harfli currency birlikte girilmeli veya ikisi de boş olmalı.";return;}if(cost.value()!=null&&!/^[A-Z]{3}$/.test(curr)){feedback.textContent="Gerçek cost currency 3 harfli olmalı.";return;}if(usage.value==="used_with_correction"&&!correctionsSelected.length){feedback.textContent="Düzeltmeyle kullanım için en az bir correction kategorisi seç.";return;}save.disabled=true;feedback.textContent="Evidence kaydediliyor ve advisory proposal threshold kontrol ediliyor…";try{const body={entry_id:freshPriceEntryId("air-learning-feedback"),tariff_usage:usage.value,actual_airline_name:airline.input.value.trim(),actual_routing_context:routing.value,actual_via_airport:routing.value==="connecting"?via.input.value.trim().toUpperCase():null,actual_service_date:service.input.value||null,actual_delivery_date:delivery.input.value||null,actual_chargeable_weight_kg:chargeable.value(),actual_cost_amount:cost.value(),actual_cost_currency:cost.value()!=null?curr:null,correction_categories:usage.value==="used_as_quoted"?[]:correctionsSelected,evidence_source:source.value,source_reference:ref.input.value.trim(),note:note.value.trim(),supersedes_feedback_id:current?.feedback_id||null};const out=await api(`/mina-jobs/${encodeURIComponent(jobId)}/air-learning-feedback`,{method:"POST",body:JSON.stringify(body)});feedback.textContent=`Evidence kaydedildi · ${out.derivation?.eligible_feedback_count||0} uygun örnek · ${out.derivation?.proposed_fact_count||0} yeni proposal · authority yok`;await refresh();}catch(e){feedback.textContent=e.message||String(e);save.disabled=false;}});
    form.append(save,feedback);section.append(form);
  }
  const facts=(view.route_facts||[]).slice().sort((a,b)=>String(b.created_at||"").localeCompare(String(a.created_at||"")));
  if(facts.length){const box=node("div","","operation-learning-list");facts.forEach(f=>{const card=node("div","",`operation-learning-card ${f.status||"proposed"}`);card.append(node("strong",codeLabel(f.fact_key)),node("div",learningFactValueLabel(f.value,f.value_unit),"operation-learning-value"),node("div",`${codeLabel(f.status)} · güven %${Math.round((Number(f.confidence)||0)*100)} · CONFIRMED OLSA BİLE ADVISORY ONLY`,"small muted"),node("div",`Fact ID: ${f.fact_id}`,"small muted"));appendLearningFactEvidence(card,f);if(f.status==="proposed"){const noteLabel=node("label","İnceleme notu");const reviewNote=document.createElement("textarea");reviewNote.rows=2;reviewNote.maxLength=1200;noteLabel.append(reviewNote);const fb=node("div","","muted settings-feedback");const act=async decision=>{if(!reviewNote.value.trim()){fb.textContent="İnceleme notu gerekli.";return;}try{await api(`/learning-facts/${encodeURIComponent(f.fact_id)}/${decision}`,{method:"POST",body:JSON.stringify({review_note:reviewNote.value.trim()})});await refresh();}catch(e){fb.textContent=e.message||String(e);}};const actions=node("div","","actions");actions.append(actionButton("Advisory'yi Onayla","approve",()=>act("confirm")),actionButton("Reddet","reject",()=>act("reject")));card.append(noteLabel,actions,fb);}else if(f.reviewed_at)card.append(node("div",`İnceleme: ${f.review_note||"-"} · ${f.reviewed_by||"-"} · ${formatDate(f.reviewed_at)}`,"small muted"));box.append(card);});section.append(box);}
  if(current){const derive=actionButton("Route Öğrenimini Yeniden Türet","",async()=>{derive.disabled=true;try{await api(`/mina-jobs/${encodeURIComponent(jobId)}/derive-air-learning`,{method:"POST"});await refresh();}catch(e){section.append(node("div",e.message||String(e),"error"));derive.disabled=false;}});section.append(derive,node("div",`Feedback audit geçmişi: ${(view.history||[]).length} kayıt. Superseded kayıtlar silinmez ve aggregation yalnız current kaydı kullanır.`,"small muted"));}
  container.append(section);
}
...
async function renderJob(data, jobId) {
  ...
  renderAirLearningSection(root, data, jobId, async () => loadJob(jobId));
...
function renderSupplierSettings(suppliersPayload = {}) {
  ...
  const importBox=node("div","","master-data-box");importBox.append(node("h3","XLSX / CSV Tedarikçi İçe Aktar"),node("div","Dosya yalnız doğrulama ve uygulama isteği sırasında işlenir; ham dosya kalıcı state veya audit kaydına yazılmaz. Mevcut kayıtlar sessizce değiştirilmez.","small muted"));
  const file=document.createElement("input");file.type="file";file.accept=".xlsx,.csv";const inspectResult=node("div","","settings-feedback");const mappingBox=node("div","","settings-inline-editor");let selectedFile=null,inspection=null,preview=null;
  const inspect=actionButton("Dosyayı İncele","",async()=>{selectedFile=file.files?.[0]||null;if(!selectedFile){inspectResult.textContent="Bir .xlsx veya .csv dosyası seç.";return;}inspect.disabled=true;try{const q=new URLSearchParams({file_name:selectedFile.name});inspection=await api(`/master-data/suppliers/import/inspect?${q}`,{method:"POST",headers:{"Content-Type":"application/octet-stream"},body:selectedFile});drawMapping();inspectResult.textContent=`${inspection.tables?.length||0} tablo/sayfa bulundu.`;}catch(e){inspectResult.textContent=e.message||String(e);}finally{inspect.disabled=false;}});
  function drawMapping(){mappingBox.replaceChildren();preview=null;const tableLabel=node("label","Sayfa / tablo");const table=document.createElement("select");(inspection.tables||[]).forEach(item=>{const o=document.createElement("option");o.value=item.name;o.textContent=`${item.name} · ${item.row_count} satır`;table.append(o);});tableLabel.append(table);const fields={supplier_name:"Tedarikçi adı *",contact_name:"Kontak adı",email:"E-posta",phone:"Telefon",countries:"Ülkeler",region_tags:"Bölge etiketleri",service_types:"Servis tipleri",equipment_types:"Ekipman tipleri",role:"Rol",notes:"Notlar"};const controls={};const form=node("div","","settings-two-col");form.append(tableLabel);const build=()=>{Array.from(form.querySelectorAll("label.mapping-field")).forEach(x=>x.remove());const selected=(inspection.tables||[]).find(x=>x.name===table.value);Object.entries(fields).forEach(([key,text])=>{const label=node("label",text,"mapping-field");const select=document.createElement("select");const none=document.createElement("option");none.value="";none.textContent="Eşleme yok";select.append(none);(selected?.headers||[]).forEach(header=>{const o=document.createElement("option");o.value=header;o.textContent=header;select.append(o);});select.value=selected?.suggested_mapping?.[key]||"";controls[key]=select;label.append(select);form.append(label);});};table.addEventListener("change",build);build();const result=node("pre","","customer-memory-import-result");const previewButton=actionButton("Önizleme Oluştur","primary",async()=>{const mapping=Object.fromEntries(Object.entries(controls).filter(([,control])=>control.value).map(([key,control])=>[key,control.value]));if(!mapping.supplier_name){result.textContent="Tedarikçi adı eşlemesi zorunlu.";return;}const q=new URLSearchParams({file_name:selectedFile.name,table_name:table.value,mapping_json:JSON.stringify(mapping)});previewButton.disabled=true;try{preview=await api(`/master-data/suppliers/import/preview?${q}`,{method:"POST",headers:{"Content-Type":"application/octet-stream"},body:selectedFile});result.textContent=`Geçerli: ${preview.counts.valid} · Mükerrer: ${preview.counts.duplicate} · Geçersiz: ${preview.counts.invalid}\n`+(preview.rows||[]).filter(x=>x.classification!=="valid").slice(0,20).map(x=>`Satır ${x.row_number}: ${x.classification} ${(x.reasons||[]).join(", ")}`).join("\n");apply.disabled=false;}catch(e){result.textContent=e.message||String(e);preview=null;}finally{previewButton.disabled=false;}});const apply=actionButton("Geçerli Satırları Uygula","approve",async()=>{if(!preview)return;const q=new URLSearchParams({file_name:selectedFile.name,table_name:preview.table,mapping_json:JSON.stringify(preview.mapping),preview_token:preview.preview_token});apply.disabled=true;try{const out=await api(`/master-data/suppliers/import/apply?${q}`,{method:"POST",headers:{"Content-Type":"application/octet-stream"},body:selectedFile});result.textContent=`Uygulandı: ${out.applied} · Atlanan mükerrer: ${out.skipped_duplicates} · Geçersiz: ${out.invalid}`;setTimeout(()=>loadSettings(),500);}catch(e){result.textContent=e.message||String(e);apply.disabled=false;}});apply.disabled=true;mappingBox.append(form,previewButton,apply,result);}
  importBox.append(file,inspect,inspectResult,mappingBox);panel.append(importBox);
...
function renderMailboxSettings(status = {}) {
  ...
  const defaults=status.imap_setup_defaults||{};const form=node("div","","relationship-onboarding-form");
  ...
  const hostLabel=node("label","IMAP sunucusu"); const host=document.createElement("input"); host.type="text"; host.autocomplete="off"; host.placeholder="mail.firma.com"; host.maxLength=253; host.value=status.provider==="imap"?(status.host||""):(defaults.host||""); hostLabel.append(host);
  const port=numberField("IMAP TLS portu",status.provider==="imap"?(status.port||993):(defaults.port||993),1,65535);
  const userLabel=node("label","IMAP kullanıcı adı"); const username=document.createElement("input"); username.type="text"; username.autocomplete="username"; username.maxLength=320; username.value=status.provider==="imap"?(status.mailbox_id||""):(defaults.username||""); userLabel.append(username);
  ...
  const pinLabel=node("label","TLS sertifika SHA-256 pini · isteğe bağlı"); const pin=document.createElement("input"); pin.type="text"; pin.autocomplete="off"; pin.placeholder="64 hex karakter"; pin.maxLength=95;pin.value=defaults.certificate_sha256||""; pinLabel.append(pin);
  const basic=node("div","","settings-two-col");basic.append(emailLabel,passwordLabel);form.append(basic);
  const advanced=document.createElement("details");const summary=document.createElement("summary");summary.textContent="Gelişmiş ayarlar";const grid=node("div","","settings-two-col");grid.append(hostLabel,port.label,userLabel,pinLabel);advanced.append(summary,grid);form.append(advanced);
  ...
  const save=actionButton("IMAP Bağlantısını Test Et ve Kaydet","primary",async()=>{
    ...
    if(!email.value.trim()||!password.value){feedback.textContent="Mailbox e-posta ve şifre gerekli.";return;}
    ...
    try{
      const result=await api("/mailbox/imap/configure",{method:"POST",body:JSON.stringify({
        mailbox_id:email.value.trim(),host:host.value.trim()||null,port:port.value(),username:username.value.trim()||null,password:password.value,certificate_sha256:pin.value.trim()||null,authorization_confirmed:true
      })});
...
function appendReviewedSurchargeCostPreview(row, tableReview, candidate, surchargeReview, additionalCostEvidence = [], costScopeReviews = [], unsupportedCostReviews = [], fxEvidence = [], customerProfiles = [], minaJobs = []) {
  if(!surchargeReview || surchargeReview.status!=="completed") return;
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Reviewed Surcharge Cost Preview · Additional Local Costs"),node("span","Reviewed airline surcharge ve yalnız explicit seçilen inquiry-bound local cost evidence ile partial reference cost. Local cost veya FX otomatik seçilmez; ters veya otomatik FX yok. All-in değildir · Müşteri teklifi değildir · Capacity/booking teyidi değildir.","small muted"));
  const actual=numberField("Gerçek ağırlık (kg)",100,0.01,1000000);
  const volumetric=numberField("Hacimsel ağırlık (kg)",100,0.01,1000000);
  const cargoLabel=node("label","Cargo context");const cargo=document.createElement("select");[["general_cargo","General cargo"],["special_cargo","Special cargo"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;cargo.append(o);});cargoLabel.append(cargo);
  const routingLabel=node("label","Routing context");const routing=document.createElement("select");[["direct","Direct"],["connecting","Connecting"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;routing.append(o);});routingLabel.append(routing);
  const viaLabel=node("label","Via airport (connecting ise opsiyonel)");const via=document.createElement("input");via.maxLength=3;via.placeholder="IST";viaLabel.append(via);
  const referenceDate=inboxField("Tarife referans tarihi","date");
  const evidenceInquiry=inboxField("FX inquiry reference / local-cost inquiry");
  const scopeReviewLabel=node("label","Cost scope review (coverage gate)");const scopeReviewSelect=document.createElement("select");const scopeEmpty=document.createElement("option");scopeEmpty.value="";scopeEmpty.textContent="Explicit scope review seç";scopeReviewSelect.append(scopeEmpty);costScopeReviews.forEach(x=>{const o=document.createElement("option");o.value=x.review_id;o.textContent=`${x.inquiry_reference} · ${x.scope_classification_complete?"scope classified":"scope unresolved"} · ${x.review_id}`;scopeReviewSelect.append(o);});scopeReviewLabel.append(scopeReviewSelect);
  const unsupportedReviewLabel=node("label","Unsupported cost semantics review");const unsupportedReviewSelect=document.createElement("select");const unsupportedEmpty=document.createElement("option");unsupportedEmpty.value="";unsupportedEmpty.textContent="Explicit unsupported-cost review seç";unsupportedReviewSelect.append(unsupportedEmpty);unsupportedCostReviews.forEach(x=>{const o=document.createElement("option");o.value=x.review_id;o.textContent=`${x.inquiry_reference} · ${x.unsupported_semantics_cleared?"semantics cleared":"semantics blocked"} · ${x.review_id}`;unsupportedReviewSelect.append(o);});unsupportedReviewLabel.append(unsupportedReviewSelect);
  const customerLabel=node("label","Customer pricing target");const customerSelect=document.createElement("select");const customerEmpty=document.createElement("option");customerEmpty.value="";customerEmpty.textContent="Aktif müşteri seç";customerSelect.append(customerEmpty);customerProfiles.filter(x=>x.active!==false).forEach(x=>{const o=document.createElement("option");o.value=x.customer_id;o.textContent=`${x.customer_name}${x.pricing_policy?` · customer policy ${x.pricing_policy.method}`:" · agency/default policy gerekebilir"}`;customerSelect.append(o);});customerLabel.append(customerSelect);
  const overrideMethodLabel=node("label","Quote pricing override (opsiyonel)");const overrideMethod=document.createElement("select");[["","Override yok"],["cost_markup_percentage","Cost markup %"],["gross_margin_percentage","Gross margin %"],["fixed_profit","Fixed profit"],["manual_sell_price","Manual sell price"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;overrideMethod.append(o);});overrideMethodLabel.append(overrideMethod);
  const overrideValue=numberField("Override value",0,0,1000000000);overrideValue.input.disabled=true;overrideMethod.addEventListener("change",()=>{overrideValue.input.disabled=!overrideMethod.value;});
  const fxReferenceAt=inboxField("FX reference timestamp (ISO-8601 + offset)");fxReferenceAt.input.placeholder="2026-09-13T15:00:00+03:00";
  const additionalLabel=node("label","Kullanılacak additional/local cost evidence");const additionalSelect=document.createElement("select");additionalSelect.multiple=true;additionalSelect.size=Math.min(6,Math.max(2,additionalCostEvidence.length||2));additionalCostEvidence.forEach(x=>{const o=document.createElement("option");o.value=x.evidence_id;o.textContent=`${x.inquiry_reference} · ${x.cost_category} · ${x.amount} ${x.currency}/${x.quantity_basis} · ${x.provider_name}`;additionalSelect.append(o);});additionalLabel.append(additionalSelect);
  const fxLabel=node("label","Kullanılacak FX evidence (çoklu seçim mümkün)");const fxSelect=document.createElement("select");fxSelect.multiple=true;fxSelect.size=Math.min(5,Math.max(2,fxEvidence.length||2));fxEvidence.forEach(x=>{const o=document.createElement("option");o.value=x.evidence_id;o.textContent=`${x.inquiry_reference} · 1 ${x.base_currency} = ${x.rate} ${x.quote_currency} · ${x.effective_at}`;fxSelect.append(o);});fxLabel.append(fxSelect);
  const countField=(label)=>{const l=node("label",label);const input=document.createElement("input");input.type="number";input.min="1";input.max="1000";input.step="1";input.placeholder="Gerekiyorsa açıkça gir";l.append(input);return {label:l,input};};
  const shipmentCount=countField("Shipment count (flat için)");const awbCount=countField("AWB count (flat için)");const hawbCount=countField("HAWB count (flat için)");const mawbCount=countField("MAWB count (flat için)");
  const addCount=(body,key,field)=>{if(!field.input.value.trim())return;const value=Number(field.input.value);if(!Number.isInteger(value)||value<1||value>1000)throw new Error(`${field.label.firstChild?.textContent||key}: 1–1000 arası tam sayı gerekli.`);body[key]=value;};
  const buildEvidenceContext=(body)=>{const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value);const selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);const inquiry=evidenceInquiry.input.value.trim();const ts=fxReferenceAt.input.value.trim();if(selectedAdditional.length||selectedFx.length){if(!inquiry)throw new Error("Additional/local cost veya FX evidence seçildiğinde inquiry reference gerekli.");body.inquiry_reference=inquiry;}if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}return {selectedAdditional,selectedFx};};
  const result=node("div","","small muted");
  const run=actionButton("Reviewed Cost Hesapla","",async()=>{run.disabled=true;result.textContent="Hesaplanıyor…";try{const body={actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;if(referenceDate.input.value)body.reference_date=referenceDate.input.value;buildEvidenceContext(body);addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/reviewed-surcharge-cost-preview`,{method:"POST",body:JSON.stringify(body)});const perKg=(out.included_surcharges||[]).map(x=>x.fx_evidence_id?`${x.surcharge_code}: ${x.source_surcharge_cost} ${x.source_currency} × FX ${x.fx_rate} = ${x.surcharge_cost} ${x.currency}`:`${x.surcharge_code}: ${x.applied_weight_kg} kg × ${x.rate_per_kg} = ${x.surcharge_cost} ${x.currency}`).join(" | ")||"Dahil edilen per-kg surcharge yok";const flat=(out.included_flat_surcharges||[]).map(x=>x.fx_evidence_id?`${x.surcharge_code}: ${x.source_surcharge_cost} ${x.source_currency} × FX ${x.fx_rate} = ${x.surcharge_cost} ${x.currency}`:`${x.surcharge_code}: ${x.applied_count} × ${x.amount_per_unit} = ${x.surcharge_cost} ${x.currency}`).join(" | ")||"Dahil edilen flat surcharge yok";const local=(out.included_additional_costs||[]).map(x=>x.fx_evidence_id?`${x.cost_category}/${x.provider_name}: ${x.source_additional_cost} ${x.source_currency} × FX ${x.fx_rate} = ${x.additional_cost} ${x.currency}`:`${x.cost_category}/${x.provider_name}: ${x.applied_count} × ${x.amount_per_unit} = ${x.additional_cost} ${x.currency}`).join(" | ")||"Seçilmiş local cost yok";const excluded=(out.excluded_surcharges||[]).map(x=>`${x.surcharge_code}/${x.currency}: ${x.reason}`).join(" | ")||"Hariç surcharge yok";const rounding=out.freight.rounding_review_id?(out.freight.rounding_mode==="none"?"reviewed: none":`reviewed ceiling ${out.freight.rounding_increment_kg} kg → ${out.freight.rounded_chargeable_weight_kg} kg`):"unreviewed / raw chargeable";result.textContent=`Base freight ${out.freight.recommended_base_freight} ${out.freight.currency} · Weight rounding ${rounding} · Reviewed per-kg ${out.reviewed_per_kg_surcharge_total} · Reviewed flat ${out.reviewed_flat_surcharge_total} · Selected local ${out.reviewed_additional_cost_total} · Extended partial subtotal ${out.base_plus_reviewed_surcharges_and_additional_costs} ${out.freight.currency} · Tarife ${out.tariff_validity_confirmed?"DOĞRULANDI":"DOĞRULANMADI"} · FX ${out.fx_applied?`UYGULANDI (${(out.fx_evidence_ids_used||[]).join(", ")})`:"YOK"} · ALL-IN DEĞİL · Per-kg: ${perKg} · Flat: ${flat} · Local: ${local} · Hariç: ${excluded}`;}catch(e){result.textContent=e.message||String(e);}finally{run.disabled=false;}});
  const coverageResult=node("div","","small muted");
  const coverage=actionButton("Required Local-Cost Coverage Kontrol Et","",async()=>{coverage.disabled=true;coverageResult.textContent="Coverage kontrol ediliyor…";try{const inquiry=evidenceInquiry.input.value.trim(),scopeReviewId=scopeReviewSelect.value;if(!inquiry||!scopeReviewId)throw new Error("Coverage gate için inquiry reference ve explicit cost scope review seçimi gerekli.");const body={cost_scope_review_id:scopeReviewId,inquiry_reference:inquiry,actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;if(referenceDate.input.value)body.reference_date=referenceDate.input.value;const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value);const selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;const ts=fxReferenceAt.input.value.trim();if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/cost-scope-coverage-preview`,{method:"POST",body:JSON.stringify(body)});const required=(out.required_categories||[]).join(", ")||"yok";const covered=(out.covered_required_categories||[]).join(", ")||"yok";const missing=(out.missing_required_categories||[]).join(", ")||"yok";const conflicts=(out.conflicting_not_applicable_categories||[]).join(", ")||"yok";const blockers=(out.blockers||[]).join(" | ")||"yok";coverageResult.textContent=`Required flat-cost coverage ${out.required_flat_cost_coverage_complete?"COMPLETE":"INCOMPLETE"} · Scope classification ${out.scope_classification_complete?"COMPLETE":"INCOMPLETE"} · Required: ${required} · Covered: ${covered} · Missing: ${missing} · Not-applicable conflicts: ${conflicts} · Blockers: ${blockers} · Extended partial subtotal ${out.cost_preview.base_plus_reviewed_surcharges_and_additional_costs} ${out.cost_preview.freight.currency} · ALL-IN DEĞİL · Müşteri fiyatı authority yok`; }catch(e){coverageResult.textContent=e.message||String(e);}finally{coverage.disabled=false;}});
  const coverageBox=node("div","","supplier-commercial");coverageBox.append(node("strong","Required Flat Local-Cost Coverage"),node("span","Explicit P2-37 scope review ile yalnız P2-36 tarafından gerçekten tüketilen selected local-cost evidence eşleştirilir. COMPLETE yalnız required flat kategorilerin karşılandığını söyler; /kg, yüzde, tiered, duty/tax veya başka bilinmeyen maliyetler nedeniyle all-in anlamına gelmez.","small muted"),coverage,coverageResult);
  const completenessResult=node("div","","small muted");
  const completeness=actionButton("Cost Completeness Kontrol Et","",async()=>{completeness.disabled=true;completenessResult.textContent="Cost completeness kontrol ediliyor…";try{const inquiry=evidenceInquiry.input.value.trim(),scopeReviewId=scopeReviewSelect.value,unsupportedReviewId=unsupportedReviewSelect.value;if(!inquiry||!scopeReviewId||!unsupportedReviewId)throw new Error("Cost completeness için inquiry, explicit cost-scope review ve explicit unsupported-cost review gerekli.");const body={cost_scope_review_id:scopeReviewId,unsupported_cost_semantics_review_id:unsupportedReviewId,inquiry_reference:inquiry,actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;if(referenceDate.input.value)body.reference_date=referenceDate.input.value;const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value);const selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;const ts=fxReferenceAt.input.value.trim();if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/cost-completeness-preview`,{method:"POST",body:JSON.stringify(body)});const blockers=(out.blockers||[]).join(" | ")||"yok";const surchargeBlockers=(out.blocking_surcharge_exclusions||[]).map(x=>`${x.surcharge_code}:${x.reason}`).join(", ")||"yok";completenessResult.textContent=`Cost completeness ${out.cost_completeness_confirmed?"CONFIRMED":"NOT CONFIRMED"} · Flat coverage ${out.coverage_preview.required_flat_cost_coverage_complete?"COMPLETE":"INCOMPLETE"} · Unsupported semantics ${out.unsupported_semantics_cleared?"CLEARED":"BLOCKED"} · Weight rounding ${out.airline_weight_rounding_reviewed?"REVIEWED":"UNREVIEWED"} · Surcharge blockers: ${surchargeBlockers} · Blockers: ${blockers}${out.cost_completeness_confirmed?` · Confirmed cost basis ${out.confirmed_cost_basis_amount} ${out.confirmed_cost_basis_currency}`:""} · Customer price authority YOK · ALL-IN label YOK`; }catch(e){completenessResult.textContent=e.message||String(e);}finally{completeness.disabled=false;}});
  const completenessBox=node("div","","supplier-commercial");completenessBox.append(node("strong","Cost Completeness Confirmation"),node("span","Flat coverage + unsupported semantic review + unresolved surcharge blockers + airline weight-rounding review birlikte değerlendirilir. CONFIRMED yalnız maliyet tabanının inquiry/source bağlamında tamamlandığını söyler; customer sell price, quote-send veya booking authority yaratmaz.","small muted"),completeness,completenessResult);
  const pricingResult=node("div","","small muted");
  const pricing=actionButton("Customer Price Preview Hesapla","",async()=>{pricing.disabled=true;pricingResult.textContent="Customer price preview hesaplanıyor…";try{const inquiry=evidenceInquiry.input.value.trim(),scopeReviewId=scopeReviewSelect.value,unsupportedReviewId=unsupportedReviewSelect.value,customerId=customerSelect.value;if(!inquiry||!scopeReviewId||!unsupportedReviewId||!customerId)throw new Error("Customer pricing preview için inquiry, scope review, unsupported-cost review ve aktif customer gerekli.");const body={cost_scope_review_id:scopeReviewId,unsupported_cost_semantics_review_id:unsupportedReviewId,inquiry_reference:inquiry,customer_id:customerId,actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;if(referenceDate.input.value)body.reference_date=referenceDate.input.value;const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value),selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;const ts=fxReferenceAt.input.value.trim();if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);if(overrideMethod.value){const value=Number(overrideValue.input.value);if(!Number.isFinite(value)||value<0||(overrideMethod.value==="manual_sell_price"&&value<=0)||(overrideMethod.value==="gross_margin_percentage"&&value>=100))throw new Error("Geçerli pricing override value gerekli.");body.quote_pricing_override={method:overrideMethod.value,value};}const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/customer-pricing-preview`,{method:"POST",body:JSON.stringify(body)});const policy=out.pricing_policy;const quote=out.customer_price_preview;const blockers=(out.blockers||[]).join(" | ")||"yok";pricingResult.textContent=quote?`PRICE PREVIEW · ${out.customer_name} · Cost basis ${quote.supplier_cost} ${quote.currency} · ${policy.policy_source}/${quote.markup_type} ${quote.markup_value} · Customer price ${quote.final_price} ${quote.currency} · QUOTE OLUŞMADI · SEND AUTHORITY YOK`:`Pricing ${out.pricing_status} · Blockers: ${blockers} · QUOTE OLUŞMADI · SEND AUTHORITY YOK`; }catch(e){pricingResult.textContent=e.message||String(e);}finally{pricing.disabled=false;}});
  const pricingBox=node("div","","supplier-commercial");pricingBox.append(node("strong","Air Customer Pricing Preview"),node("span","Yalnız P2-39 cost completeness CONFIRMED ise mevcut MINAI pricing resolver kullanılır: explicit quote override > verified customer master policy > agency default. Accepted-quote learning/advisory fiyat authority değildir. Bu işlem quote oluşturmaz veya göndermez.","small muted"),pricing,pricingResult);
  const learningAdvisoryResult=node("div","","small muted");
  const learningAdvisory=actionButton("Air Learning Advisory Göster","",async()=>{learningAdvisory.disabled=true;learningAdvisoryResult.textContent="Confirmed air learning aranıyor…";try{const body={routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/learning-advisory`,{method:"POST",body:JSON.stringify(body)});const facts=(out.confirmed_facts||[]).map(f=>`${codeLabel(f.fact_key)}: ${learningFactValueLabel(f.value,f.value_unit)} (güven %${Math.round((Number(f.confidence)||0)*100)})`).join(" | ");learningAdvisoryResult.textContent=facts?`${out.subject_label} · CONFIRMED ADVISORY · ${facts} · FİYAT/TARİFE/ROTA/BOOKING AUTHORITY YOK`:`${out.subject_label} · Henüz human-confirmed air advisory yok · FİYAT/TARİFE/ROTA/BOOKING AUTHORITY YOK`;}catch(e){learningAdvisoryResult.textContent=e.message||String(e);}finally{learningAdvisory.disabled=false;}});
  const learningAdvisoryBox=node("div","","supplier-commercial");learningAdvisoryBox.append(node("strong","Air Learning Advisory"),node("span","Aynı airport-pair + airline + routing için human-confirmed geçmiş outcome gözlemlerini gösterir. Bu sinyaller mevcut tariff satırını, cost hesabını, müşteri fiyatını veya route seçimini değiştirmez.","small muted"),learningAdvisory,learningAdvisoryResult);
  const readinessResult=node("div","","small muted");
  const readiness=actionButton("Operational Readiness Kontrol Et","",async()=>{readiness.disabled=true;readinessResult.textContent="Kontrol ediliyor…";try{const inquiry=evidenceInquiry.input.value.trim();if(!referenceDate.input.value||!inquiry)throw new Error("Operational readiness için service date ve inquiry reference gerekli.");const body={inquiry_reference:inquiry,service_date:referenceDate.input.value,actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value);const selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);const ts=fxReferenceAt.input.value.trim();if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/operational-readiness-preview`,{method:"POST",body:JSON.stringify(body)});const blockers=(out.operational_blockers||[]).join(", ")||"yok";const rounding=out.cost_preview.freight.rounding_review_id?(out.cost_preview.freight.rounding_mode==="none"?"reviewed: none":`reviewed ceiling ${out.cost_preview.freight.rounding_increment_kg} kg → ${out.cost_preview.freight.rounded_chargeable_weight_kg} kg`):"unreviewed / raw chargeable";readinessResult.textContent=`Operational evidence ${out.operational_evidence_complete?"COMPLETE":"INCOMPLETE"} · Tarife ${out.tariff_validity_confirmed?"OK":"EKSİK"} · Capacity ${out.capacity_confirmed?"OK":"EKSİK"} · Schedule ${out.schedule_confirmed?"OK":"EKSİK"} · Weight rounding ${rounding} · Selected local ${out.cost_preview.reviewed_additional_cost_total} · Extended partial subtotal ${out.cost_preview.base_plus_reviewed_surcharges_and_additional_costs} ${out.cost_preview.freight.currency} · FX ${out.fx_applied?`UYGULANDI (${(out.fx_evidence_ids_used||[]).join(", ")})`:"YOK"} · Blockers: ${blockers} · ALL-IN DEĞİL · Müşteri teklifi hazır değil · Booking authority yok`; }catch(e){readinessResult.textContent=e.message||String(e);}finally{readiness.disabled=false;}});
  const readinessBox=node("div","","supplier-commercial");readinessBox.append(node("strong","Operational Readiness Preview"),node("span","Exact inquiry + service date + source-bound tariff validity + capacity/schedule evidence kontrol edilir. Seçilmiş local costs subtotalı genişletebilir ama customer quote veya booking authority yaratmaz.","small muted"),readiness,readinessResult);
  const qrPickup=inboxField("Quote readiness pickup address");const qrDelivery=inboxField("Quote readiness delivery address");const qrCommodity=inboxField("Commodity");
  const qrQty=numberField("Package quantity",1,1,10000);qrQty.input.step="1";const qrLength=numberField("Package length cm",100,0.01,100000);const qrWidth=numberField("Package width cm",80,0.01,100000);const qrHeight=numberField("Package height cm",60,0.01,100000);
  const qrReady=inboxField("Cargo ready date","date");const qrDeadline=inboxField("Customer required delivery date (opsiyonel)","date");
  const boolSelect=(label)=>{const l=node("label",label);const el=document.createElement("select");[["","Explicit seç"],["false","Hayır"],["true","Evet"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;el.append(o);});l.append(el);return {label:l,select:el};};
  const qrAdr=boolSelect("ADR / dangerous goods?");const qrAdrClass=inboxField("ADR class (ADR ise)");const qrTemp=boolSelect("Temperature controlled?");const qrTempReq=inboxField("Temperature requirement (gerekirse)");const qrHighValue=boolSelect("High value?");
  const qrGrid=node("div","","settings-two-col");qrGrid.append(qrPickup.label,qrDelivery.label,qrCommodity.label,qrReady.label,qrDeadline.label,qrQty.label,qrLength.label,qrWidth.label,qrHeight.label,qrAdr.label,qrAdrClass.label,qrTemp.label,qrTempReq.label,qrHighValue.label);
  const qrResult=node("div","","small muted");
  const quoteReadiness=actionButton("Air Quote Readiness Kontrol Et","",async()=>{quoteReadiness.disabled=true;qrResult.textContent="Quote readiness kontrol ediliyor…";try{const inquiry=evidenceInquiry.input.value.trim(),scopeReviewId=scopeReviewSelect.value,unsupportedReviewId=unsupportedReviewSelect.value,customerId=customerSelect.value;if(!inquiry||!scopeReviewId||!unsupportedReviewId||!customerId||!referenceDate.input.value)throw new Error("Quote readiness için inquiry, reviews, customer ve service date gerekli.");if(!qrPickup.input.value.trim()||!qrDelivery.input.value.trim()||!qrCommodity.input.value.trim()||!qrReady.input.value)throw new Error("Pickup/delivery address, commodity ve cargo ready date gerekli.");if(!qrAdr.select.value||!qrTemp.select.value||!qrHighValue.select.value)throw new Error("ADR, temperature-control ve high-value durumlarını explicit seç.");const qty=Number(qrQty.input.value),length=Number(qrLength.input.value),width=Number(qrWidth.input.value),height=Number(qrHeight.input.value);if(!Number.isInteger(qty)||qty<1||![length,width,height].every(v=>Number.isFinite(v)&&v>0))throw new Error("Pozitif package quantity ve dimensions gerekli.");const profile=customerProfiles.find(x=>x.customer_id===customerId);if(!profile)throw new Error("Aktif customer master bulunamadı.");const body={cost_scope_review_id:scopeReviewId,unsupported_cost_semantics_review_id:unsupportedReviewId,inquiry_reference:inquiry,customer_id:customerId,service_date:referenceDate.input.value,cargo_context:cargo.value,routing_context:routing.value,shipment:{customer_name:profile.customer_name,pickup_address:qrPickup.input.value.trim(),delivery_address:qrDelivery.input.value.trim(),commodity:qrCommodity.input.value.trim(),gross_weight_kg:actual.value(),transport_mode:"air",cargo_ready_date:qrReady.input.value,required_delivery_date:qrDeadline.input.value||null,is_adr:qrAdr.select.value==="true",adr_class:qrAdrClass.input.value.trim()||null,is_temperature_controlled:qrTemp.select.value==="true",temperature_requirement:qrTempReq.input.value.trim()||null,is_high_value:qrHighValue.select.value==="true",packages:[{package_type:"pallet",quantity:qty,length_cm:length,width_cm:width,height_cm:height}]}};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value),selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;const ts=fxReferenceAt.input.value.trim();if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);if(overrideMethod.value){const value=Number(overrideValue.input.value);if(!Number.isFinite(value)||value<0||(overrideMethod.value==="manual_sell_price"&&value<=0)||(overrideMethod.value==="gross_margin_percentage"&&value>=100))throw new Error("Geçerli pricing override value gerekli.");body.quote_pricing_override={method:overrideMethod.value,value};}const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/quote-readiness-preview`,{method:"POST",body:JSON.stringify(body)});const blockers=(out.blockers||[]).join(" | ")||"yok";const quote=out.pricing_preview?.customer_price_preview;qrResult.textContent=`Quote readiness ${out.quote_ready?"READY":"BLOCKED"} · Shipment inputs ${out.shipment_inputs_complete?"COMPLETE":"INCOMPLETE"} · Pricing ${out.pricing_complete?"READY":"BLOCKED"} · Operational evidence ${out.operational_evidence_complete?"COMPLETE":"INCOMPLETE"} · Regulatory ${out.regulatory_compliance_clear?"CLEAR":"BLOCKED"} · Delivery deadline ${out.delivery_deadline_status}${quote?` · Customer price ${quote.final_price} ${quote.currency}`:""} · Blockers: ${blockers} · QUOTE HENÜZ OLUŞMADI · SEND AUTHORITY YOK · BOOKING AUTHORITY YOK`; }catch(e){qrResult.textContent=e.message||String(e);}finally{quoteReadiness.disabled=false;}});
  const quoteReadinessBox=node("div","","supplier-commercial");quoteReadinessBox.append(node("strong","Air Quote Readiness Gate"),node("span","Cost completeness + customer price + exact service-date tariff/capacity/schedule + shipment inputs birlikte doğrulanır. Customer delivery deadline yoksa süreç devam eder; varsa expected-delivery evidence deadline'ı karşılamalıdır. READY yalnız sonraki quote oluşturma adımına izin verir; bu ekran quote oluşturmaz/göndermez ve booking yapmaz.","small muted"),qrGrid,quoteReadiness,qrResult);
  const airJobLabel=node("label","Air MINA işi");const airJobSelect=document.createElement("select");const airJobEmpty=document.createElement("option");airJobEmpty.value="";airJobEmpty.textContent="MINA işi seç";airJobSelect.append(airJobEmpty);minaJobs.filter(j=>j.transport_mode==="air"&&j.job_kind==="price_request"&&!j.is_closed&&["inquiry_confirmed","pricing","quote_ready"].includes(j.stage)).forEach(j=>{const o=document.createElement("option");o.value=j.job_id;o.textContent=`${j.mina_code} · ${j.customer_name} · ${j.route} · ${codeLabel(j.stage)}`;airJobSelect.append(o);});airJobLabel.append(airJobSelect);
  const prepareResult=node("div","","small muted");
  const prepareAirQuote=actionButton("QuoteCase + Human Approval Hazırla","approve",async()=>{prepareAirQuote.disabled=true;prepareResult.textContent="Air quote case hazırlanıyor…";try{const jobId=airJobSelect.value,inquiry=evidenceInquiry.input.value.trim(),scopeReviewId=scopeReviewSelect.value,unsupportedReviewId=unsupportedReviewSelect.value,customerId=customerSelect.value;if(!jobId||!inquiry||!scopeReviewId||!unsupportedReviewId||!customerId||!referenceDate.input.value)throw new Error("Preparation için MINA işi, inquiry, reviews, customer ve service date gerekli.");const body={review_id:tableReview.review_id,candidate_id:candidate.candidate_id,cost_scope_review_id:scopeReviewId,unsupported_cost_semantics_review_id:unsupportedReviewId,inquiry_reference:inquiry,customer_id:customerId,service_date:referenceDate.input.value,cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;const selectedAdditional=Array.from(additionalSelect.selectedOptions).map(o=>o.value),selectedFx=Array.from(fxSelect.selectedOptions).map(o=>o.value);if(selectedAdditional.length)body.additional_cost_evidence_ids=selectedAdditional;const ts=fxReferenceAt.input.value.trim();if(selectedFx.length){if(!/(Z|[+-]\d{2}:\d{2})$/.test(ts))throw new Error("FX evidence seçildiğinde timezone-offset'li FX reference timestamp gerekli.");body.fx_evidence_ids=selectedFx;body.fx_reference_at=ts;}else if(ts){throw new Error("FX reference timestamp yalnız explicit FX evidence seçimi ile kullanılabilir.");}addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);if(overrideMethod.value){const value=Number(overrideValue.input.value);if(!Number.isFinite(value)||value<0||(overrideMethod.value==="manual_sell_price"&&value<=0)||(overrideMethod.value==="gross_margin_percentage"&&value>=100))throw new Error("Geçerli pricing override value gerekli.");body.quote_pricing_override={method:overrideMethod.value,value};}const out=await api(`/mina-jobs/${encodeURIComponent(jobId)}/air-quote/prepare`,{method:"POST",body:JSON.stringify(body)});if(out.status==="blocked"){prepareResult.textContent=`PREPARATION BLOCKED · ${(out.readiness?.blockers||[]).join(" | ")||"readiness geçmedi"} · HİÇBİR QUOTE CASE YAZILMADI`;return;}prepareResult.textContent=`${out.status==="existing"?"MEVCUT":"OLUŞTURULDU"} · ${out.mina_job?.mina_code||"MINA"} · QuoteCase ${out.quote_case?.case_id||"-"} · Approval ${out.quote_approval?.approval_status||"pending"} · HUMAN APPROVAL GEREKLİ · SEND OTOMATİK DEĞİL · BOOKING YOK`; }catch(e){prepareResult.textContent=e.message||String(e);}finally{prepareAirQuote.disabled=false;}});
  const prepareGrid=node("div","","settings-two-col");prepareGrid.append(airJobLabel);
  const preparationBox=node("div","","supplier-commercial");preparationBox.append(node("strong","Air Quote Preparation · Durable Human Approval"),node("span","Backend seçilen MINA job'ın authoritative shipment snapshot'ını P2-41 ile yeniden doğrular. READY ise mevcut QuoteCase + QuoteApproval(pending) zincirini oluşturur; aynı evidence tekrarında idempotent davranır. Bu aksiyon teklif göndermez ve booking yapmaz.","small muted"),prepareGrid,prepareAirQuote,prepareResult);
  const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label,cargoLabel,routingLabel,viaLabel,referenceDate.label,evidenceInquiry.label,scopeReviewLabel,unsupportedReviewLabel,customerLabel,overrideMethodLabel,overrideValue.label,additionalLabel,fxReferenceAt.label,fxLabel,shipmentCount.label,awbCount.label,hawbCount.label,mawbCount.label);box.append(grid,node("span","Additional/local cost kayıtları otomatik seçilmez. Seçim exact inquiry/source ile eşleşir ve quantity basis için matching count ister. Cross-currency surcharge FX yoksa currency_mismatch_no_fx olarak hariç kalır; seçilmiş cross-currency local cost ise matching FX yoksa fail-closed olur. Inverse/nearest/latest kur yoktur. Tarife referans tarihi boşsa bugünün tarihi varsayılmaz. Count alanları boş bırakılırsa hiçbir count varsayılmaz. Bu subtotal hâlâ all-in değildir.","small muted"),run,result,coverageBox,completenessBox,pricingBox,learningAdvisoryBox,readinessBox,quoteReadinessBox,preparationBox);row.append(box);
}

function appendAirWeightRoundingReview(card, item, review = null) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Airline Weight Rounding"),node("span","Exact tariff source kuralıdır. Yalnız chargeable weight'i tariff break değerlendirmesinden önce etkiler; surcharge basis, customer price veya booking authority yaratmaz.","small muted"));
  if(review){const rule=review.rounding_mode==="none"?"Explicit no rounding":`Ceiling · ${review.increment_kg} kg increment`;box.append(node("span",`${rule} · ${review.reviewed_by} · ${review.review_note}`,"small muted"));card.append(box);return;}
  const modeLabel=node("label","Rounding rule");const mode=document.createElement("select");[["none","No rounding (explicit)"],["ceiling","Ceiling / yukarı yuvarla"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;mode.append(o);});modeLabel.append(mode);
  const increment=numberField("Increment (kg)",0.5,0.01,100);increment.input.step="0.01";increment.input.disabled=true;mode.addEventListener("change",()=>{increment.input.disabled=mode.value!=="ceiling";});
  const note=inboxField("Rounding review note");note.input.placeholder="Exact source/airline evidence hangi rounding kuralını gösteriyor?";
  const fb=node("div","","muted settings-feedback");const save=actionButton("Weight Rounding Doğrula","",async()=>{if(!note.input.value.trim()){fb.textContent="Review note gerekli.";return;}let inc=null;if(mode.value==="ceiling"){inc=Number(increment.input.value);if(!Number.isFinite(inc)||inc<=0||inc>100){fb.textContent="Ceiling için 0–100 kg arası pozitif increment gerekli.";return;}}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/weight-rounding-review`,{method:"POST",body:JSON.stringify({rounding_mode:mode.value,increment_kg:inc,review_note:note.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(modeLabel,increment.label,note.label);box.append(grid,save,fb);card.append(box);
}

function appendAirCostScopeReview(card, item, reviews = []) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Air Cost Scope Requirements Review"),node("span","Current inquiry için local-cost alanlarının hangilerinin required / not applicable / unresolved olduğunu sınıflandırır. Bu yalnız kapsam evidence'ıdır; cost completeness, all-in veya customer-pricing authority yaratmaz.","small muted"));
  const labels={pickup:"Pickup",origin_handling:"Origin handling",origin_terminal:"Origin terminal",documentation:"Documentation",customs_service_fee:"Customs service fee",destination_handling:"Destination handling",destination_terminal:"Destination terminal",delivery:"Delivery",other:"Other known local costs"};
  reviews.slice(0,5).forEach(x=>{const unresolved=(x.unresolved_categories||[]).map(k=>labels[k]||k);const required=(x.required_categories||[]).map(k=>labels[k]||k);box.append(node("span",`${x.inquiry_reference} · Scope classification ${x.scope_classification_complete?"COMPLETE":"INCOMPLETE"} · Required: ${required.join(", ")||"yok"} · Unresolved: ${unresolved.join(", ")||"yok"} · ${x.reviewed_by}`,"small muted"));});
  const inquiry=inboxField("Inquiry reference");
  const overall=inboxField("Scope review note");overall.input.placeholder="Bu inquiry'nin local-cost kapsamı hangi servis/evidence bağlamıyla değerlendirildi?";
  const categories=["pickup","origin_handling","origin_terminal","documentation","customs_service_fee","destination_handling","destination_terminal","delivery","other"];
  const editors=categories.map(category=>{const statusLabel=node("label",`${labels[category]} status`);const status=document.createElement("select");[["unresolved","Unresolved"],["required","Required"],["not_applicable","Not applicable"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;status.append(o);});statusLabel.append(status);const rationale=inboxField(`${labels[category]} rationale`);rationale.input.placeholder="Neden required / not applicable / unresolved?";return {category,status,statusLabel,rationale};});
  const grid=node("div","","settings-two-col");grid.append(inquiry.label,overall.label);editors.forEach(x=>grid.append(x.statusLabel,x.rationale.label));
  const fb=node("div","","muted settings-feedback");
  const save=actionButton("Cost Scope Review Kaydet","",async()=>{if(!inquiry.input.value.trim()||!overall.input.value.trim()){fb.textContent="Inquiry reference ve genel review note gerekli.";return;}const requirements=[];for(const x of editors){const rationale=x.rationale.input.value.trim();if(!rationale){fb.textContent=`${labels[x.category]} için rationale gerekli.`;return;}requirements.push({category:x.category,status:x.status.value,rationale});}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/cost-scope-reviews`,{method:"POST",body:JSON.stringify({entry_id:freshPriceEntryId("web-air-cost-scope"),inquiry_reference:inquiry.input.value.trim(),requirements,review_note:overall.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  box.append(grid,node("span","Her dokuz kategori tam olarak bir kez sınıflandırılır; varsayılan unresolved'dır. Scope classification complete olsa bile seçilmiş maliyetlerin tam olduğu veya quote'un all-in olduğu sonucuna varılmaz.","small muted"),save,fb);card.append(box);
}

function appendAirUnsupportedCostSemanticsReview(card, item, reviews = []) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Unsupported Cost Semantics Review"),node("span","Flat local-cost modelinin dışında kalabilecek /kg, yüzde, minimum/tiered, duty/tax ve diğer unmodeled maliyetleri inquiry bazında explicit olarak sınıflandırır. Varsayılan unresolved; bu review tek başına customer price authority yaratmaz.","small muted"));
  const labels={weight_based_additional_cost:"Weight-based additional cost",percentage_additional_cost:"Percentage-based additional cost",minimum_tiered_formula_additional_cost:"Minimum / tiered / formula additional cost",customs_duties_taxes:"Customs duties / taxes / government charges",other_unmodeled_cost:"Other unmodeled cost"};
  reviews.slice(0,5).forEach(x=>box.append(node("span",`${x.inquiry_reference} · Semantics ${x.semantics_classification_complete?"CLASSIFIED":"INCOMPLETE"} · Cleared ${x.unsupported_semantics_cleared?"YES":"NO"} · Applicable unresolved: ${(x.applicable_unresolved_semantics||[]).map(k=>labels[k]||k).join(", ")||"yok"} · ${x.reviewed_by}`,"small muted")));
  const inquiry=inboxField("Inquiry reference");const overall=inboxField("Unsupported-cost review note");overall.input.placeholder="Bu inquiry'de flat model dışındaki maliyet semantikleri hangi evidence/servis kapsamına göre değerlendirildi?";
  const semantics=["weight_based_additional_cost","percentage_additional_cost","minimum_tiered_formula_additional_cost","customs_duties_taxes","other_unmodeled_cost"];
  const editors=semantics.map(semantic=>{const statusLabel=node("label",`${labels[semantic]} status`);const status=document.createElement("select");[["unresolved","Unresolved"],["not_applicable","Not applicable"],["applicable_unresolved","Applicable / unresolved"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;status.append(o);});statusLabel.append(status);const rationale=inboxField(`${labels[semantic]} rationale`);rationale.input.placeholder="Neden not applicable / applicable unresolved / unresolved?";return {semantic,status,statusLabel,rationale};});
  const grid=node("div","","settings-two-col");grid.append(inquiry.label,overall.label);editors.forEach(x=>grid.append(x.statusLabel,x.rationale.label));const fb=node("div","","muted settings-feedback");
  const save=actionButton("Unsupported Cost Semantics Review Kaydet","",async()=>{if(!inquiry.input.value.trim()||!overall.input.value.trim()){fb.textContent="Inquiry reference ve genel review note gerekli.";return;}const requirements=[];for(const x of editors){const rationale=x.rationale.input.value.trim();if(!rationale){fb.textContent=`${labels[x.semantic]} için rationale gerekli.`;return;}requirements.push({semantic:x.semantic,status:x.status.value,rationale});}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/unsupported-cost-semantics-reviews`,{method:"POST",body:JSON.stringify({entry_id:freshPriceEntryId("web-air-unsupported-cost"),inquiry_reference:inquiry.input.value.trim(),requirements,review_note:overall.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  box.append(grid,node("span","Beş semantic tam olarak bir kez sınıflandırılır; varsayılan unresolved'dır. Applicable/unresolved kalan herhangi bir semantic cost completeness'i bloke eder.","small muted"),save,fb);card.append(box);
}

function appendAirAdditionalCostEvidence(card, item, evidence = []) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Additional / Local Cost Evidence"),node("span","Yalnız current inquiry + exact air source context için flat evidence kaydeder. Bu aşama cost preview'a otomatik eklemez; /kg, yüzde, minimum/tiered ücretler ve customs duties/taxes desteklenmez.","small muted"));
  const categoryLabel={pickup:"Pickup",origin_handling:"Origin handling",origin_terminal:"Origin terminal",documentation:"Documentation",customs_service_fee:"Customs service fee",destination_handling:"Destination handling",destination_terminal:"Destination terminal",delivery:"Delivery",other:"Other"};
  const quantityLabel={per_shipment:"shipment",per_awb:"AWB",per_hawb:"HAWB",per_mawb:"MAWB"};
  evidence.slice(0,8).forEach(x=>box.append(node("span",`${x.inquiry_reference} · ${categoryLabel[x.cost_category]||x.cost_category} · ${x.amount} ${x.currency} / ${quantityLabel[x.quantity_basis]||x.quantity_basis} · ${x.provider_name} · ${x.evidence_source}/${x.evidence_reference}`,"small muted")));
  const inquiry=inboxField("Inquiry reference");const provider=inboxField("Provider / supplier");
  const categoryL=node("label","Cost category");const category=document.createElement("select");[["pickup","Pickup"],["origin_handling","Origin handling"],["origin_terminal","Origin terminal"],["documentation","Documentation"],["customs_service_fee","Customs service fee (duties/taxes değil)"],["destination_handling","Destination handling"],["destination_terminal","Destination terminal"],["delivery","Delivery"],["other","Other"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;category.append(o);});categoryL.append(category);
  const amount=numberField("Amount",1,0.01,1000000000);amount.input.step="0.01";const currency=inboxField("Currency");currency.input.maxLength=3;currency.input.placeholder="USD";
  const quantityL=node("label","Quantity basis");const quantity=document.createElement("select");[["per_shipment","Per shipment"],["per_awb","Per AWB"],["per_hawb","Per HAWB"],["per_mawb","Per MAWB"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;quantity.append(o);});quantityL.append(quantity);
  const sourceL=node("label","Evidence source");const source=document.createElement("select");[["email","E-posta"],["phone","Telefon"],["whatsapp","WhatsApp"],["portal","Portal"],["manual_document","Manuel doküman"],["other","Diğer"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;source.append(o);});sourceL.append(source);
  const ref=inboxField("Evidence reference");const note=inboxField("Evidence note");const fb=node("div","","muted settings-feedback");
  const save=actionButton("Additional Cost Evidence Kaydet","",async()=>{const curr=currency.input.value.trim().toUpperCase(),amt=Number(amount.input.value);if(!inquiry.input.value.trim()||!provider.input.value.trim()||!/^[A-Z]{3}$/.test(curr)||!Number.isFinite(amt)||amt<=0||!ref.input.value.trim()||!note.input.value.trim()){fb.textContent="Inquiry, provider, pozitif amount, 3-harfli currency, reference ve note gerekli.";return;}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/additional-cost-evidence`,{method:"POST",body:JSON.stringify({entry_id:freshPriceEntryId("web-air-additional-cost"),inquiry_reference:inquiry.input.value.trim(),cost_category:category.value,provider_name:provider.input.value.trim(),amount:amt,currency:curr,quantity_basis:quantity.value,evidence_source:source.value,evidence_reference:ref.input.value.trim(),evidence_note:note.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(inquiry.label,provider.label,categoryL,amount.label,currency.label,quantityL,sourceL,ref.label,note.label);box.append(grid,save,fb);card.append(box);
}

function appendAirFxEvidence(card, item, evidence = []) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","FX Rate Evidence"),node("span","Yalnız inquiry + exact tariff source evidence'ıdır. Kur yönü: 1 BASE = rate QUOTE. Reviewed cost preview yalnız explicit seçilmiş matching evidence'ı tüketebilir; otomatik/inverse FX yoktur.","small muted"));
  evidence.slice(0,5).forEach(x=>box.append(node("span",`${x.inquiry_reference} · 1 ${x.base_currency} = ${x.rate} ${x.quote_currency} · ${x.effective_at} · ${x.evidence_source}/${x.evidence_reference}`,"small muted")));
  const inquiry=inboxField("Inquiry reference");const base=inboxField("Base currency");base.input.maxLength=3;base.input.placeholder="USD";const quote=inboxField("Quote currency");quote.input.maxLength=3;quote.input.placeholder="EUR";
  const rate=numberField("Rate (1 BASE = ? QUOTE)",1,0.000001,1000000);rate.input.step="0.000001";
  const effective=inboxField("Effective timestamp (ISO-8601 + offset)");effective.input.placeholder="2026-09-13T15:00:00+03:00";
  const sourceLabel=node("label","Evidence source");const source=document.createElement("select");[["bank","Banka"],["central_bank","Merkez bankası"],["airline","Havayolu"],["manual_document","Manuel doküman"],["other","Diğer"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;source.append(o);});sourceLabel.append(source);
  const ref=inboxField("Evidence reference");const note=inboxField("Evidence note");const fb=node("div","","muted settings-feedback");
  const save=actionButton("FX Evidence Kaydet","",async()=>{const b=base.input.value.trim().toUpperCase(),q=quote.input.value.trim().toUpperCase(),r=Number(rate.input.value),ts=effective.input.value.trim();if(!inquiry.input.value.trim()||!/^[A-Z]{3}$/.test(b)||!/^[A-Z]{3}$/.test(q)||b===q||!Number.isFinite(r)||r<=0||!/(Z|[+-]\d{2}:\d{2})$/.test(ts)||!ref.input.value.trim()||!note.input.value.trim()){fb.textContent="Inquiry, farklı 3-harfli currency pair, pozitif rate, timezone-offset'li timestamp, reference ve note gerekli.";return;}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/fx-rate-evidence`,{method:"POST",body:JSON.stringify({entry_id:freshPriceEntryId("web-air-fx"),inquiry_reference:inquiry.input.value.trim(),base_currency:b,quote_currency:q,rate:r,effective_at:ts,evidence_source:source.value,evidence_reference:ref.input.value.trim(),evidence_note:note.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(inquiry.label,base.label,quote.label,rate.label,effective.label,sourceLabel,ref.label,note.label);box.append(grid,save,fb);card.append(box);
}

function appendAirAvailabilityEvidence(card, item, confirmations = []) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Kapasite / Schedule Evidence"),node("span","Shipment/inquiry bazlıdır; tarifeye kalıcı availability özelliği yazmaz ve booking authority yaratmaz.","small muted"));
  confirmations.slice(0,5).forEach(x=>box.append(node("span",`${x.inquiry_reference} · ${x.destination_code} · ${x.service_date}${x.expected_delivery_date?` → expected delivery ${x.expected_delivery_date}`:""} · capacity ${x.capacity_status} · schedule ${x.schedule_status}${x.flight_reference?` · ${x.flight_reference}`:""} · ${x.evidence_channel}/${x.evidence_reference}`,"small muted")));
  const inquiry=inboxField("Inquiry reference");const destination=inboxField("Destination IATA");destination.input.maxLength=3;
  const routingLabel=node("label","Routing");const routing=document.createElement("select");[["direct","Direct"],["connecting","Connecting"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;routing.append(o);});routingLabel.append(routing);
  const via=inboxField("Via airport (opsiyonel)");via.input.maxLength=3;const serviceDate=inboxField("Service date","date");const expectedDelivery=inboxField("Expected delivery date (müşteri deadline verdiyse gerekli)","date");
  const capacityLabel=node("label","Capacity");const capacity=document.createElement("select");[["available","Available"],["unavailable","Unavailable"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;capacity.append(o);});capacityLabel.append(capacity);
  const scheduleLabel=node("label","Schedule");const schedule=document.createElement("select");[["confirmed","Confirmed"],["not_confirmed","Not confirmed"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;schedule.append(o);});scheduleLabel.append(schedule);
  const flight=inboxField("Flight reference (schedule confirmed ise)");const channelLabel=node("label","Evidence channel");const channel=document.createElement("select");[["email","E-posta"],["phone","Telefon"],["whatsapp","WhatsApp"],["portal","Portal"],["other","Diğer"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;channel.append(o);});channelLabel.append(channel);
  const evidenceRef=inboxField("Evidence reference");const note=inboxField("Evidence note");const fb=node("div","","muted settings-feedback");
  const save=actionButton("Kapasite / Schedule Teyidi Kaydet","",async()=>{const dest=destination.input.value.trim().toUpperCase();const viaCode=via.input.value.trim().toUpperCase();if(!inquiry.input.value.trim()||!/^[A-Z]{3}$/.test(dest)||!serviceDate.input.value||!evidenceRef.input.value.trim()||!note.input.value.trim()){fb.textContent="Inquiry, destination, service date, evidence reference ve note gerekli.";return;}if(routing.value==="direct"&&viaCode){fb.textContent="Direct routing via airport taşıyamaz.";return;}if(viaCode&&!/^[A-Z]{3}$/.test(viaCode)){fb.textContent="Via airport 3 harfli IATA kodu olmalı.";return;}if(schedule.value==="not_confirmed"&&(flight.input.value.trim()||expectedDelivery.input.value)){fb.textContent="Schedule not confirmed iken flight veya expected-delivery evidence girilemez.";return;}save.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/availability-confirmations`,{method:"POST",body:JSON.stringify({entry_id:freshPriceEntryId("web-air-availability"),inquiry_reference:inquiry.input.value.trim(),destination_code:dest,routing_context:routing.value,via_airport:viaCode||null,service_date:serviceDate.input.value,expected_delivery_date:expectedDelivery.input.value||null,capacity_status:capacity.value,schedule_status:schedule.value,flight_reference:flight.input.value.trim()||null,evidence_channel:channel.value,evidence_reference:evidenceRef.input.value.trim(),evidence_note:note.input.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(inquiry.label,destination.label,routingLabel,via.label,serviceDate.label,expectedDelivery.label,capacityLabel,scheduleLabel,flight.label,channelLabel,evidenceRef.label,note.label);box.append(grid,save,fb);card.append(box);
}

function renderAirRateSourceSettings(payload = {}, validityPayload = {}, roundingPayload = {}, additionalCostPayload = {}, costScopePayload = {}, unsupportedCostPayload = {}, fxPayload = {}, availabilityPayload = {}, reviewsPayload = {}, tablePayload = {}, surchargePayload = {}, customerProfiles = [], minaJobs = []) {
  const panel=node("section","","settings-panel"); const h=node("div","","settings-heading");
  h.append(node("h2","Havayolu Listeleri"),node("p","Ticari havayolu tarife PDF'lerini shadow-learning için kaydeder; yapısal etiketleri, numeric satırları ve açık unit içeren surcharge tutar adaylarını yerelde çıkarır. Quote üretmez ve express tarifesi kabul etmez.","muted")); panel.append(h);
  panel.append(node("div","PDF ticari sır olarak korunur. Bu ekran müşteri satış fiyatı üretmez. Yapı, satır ve surcharge çıkarımı yerel/deterministik çalışır; AI parser çağrılmaz ve tam extracted text state/audit kaydına yazılmaz.","notice"));
  const sources=(payload.rate_sources||[]).slice(); const validityReviews=(validityPayload.reviews||[]).slice(); const roundingReviews=(roundingPayload.reviews||[]).slice(); const roundingBySource=Object.fromEntries(roundingReviews.map(r=>[r.source_id,r])); const additionalCostEvidence=(additionalCostPayload.evidence||[]).slice(); const additionalCostBySource=Object.groupBy?Object.groupBy(additionalCostEvidence,x=>x.source_id):additionalCostEvidence.reduce((m,x)=>((m[x.source_id]??=[]).push(x),m),{}); const costScopeReviews=(costScopePayload.reviews||[]).slice(); const costScopeBySource=Object.groupBy?Object.groupBy(costScopeReviews,x=>x.source_id):costScopeReviews.reduce((m,x)=>((m[x.source_id]??=[]).push(x),m),{}); const unsupportedCostReviews=(unsupportedCostPayload.reviews||[]).slice(); const unsupportedCostBySource=Object.groupBy?Object.groupBy(unsupportedCostReviews,x=>x.source_id):unsupportedCostReviews.reduce((m,x)=>((m[x.source_id]??=[]).push(x),m),{}); const fxEvidence=(fxPayload.evidence||[]).slice(); const fxBySource=Object.groupBy?Object.groupBy(fxEvidence,x=>x.source_id):fxEvidence.reduce((m,x)=>((m[x.source_id]??=[]).push(x),m),{}); const availabilityConfirmations=(availabilityPayload.confirmations||[]).slice(); const availabilityBySource=Object.groupBy?Object.groupBy(availabilityConfirmations,x=>x.source_id):availabilityConfirmations.reduce((m,x)=>((m[x.source_id]??=[]).push(x),m),{}); const reviews=(reviewsPayload.reviews||[]).slice(); const tableReviews=(tablePayload.reviews||[]).slice(); const surchargeReviews=(surchargePayload.reviews||[]).slice(); const validityReviewBySource=Object.fromEntries(validityReviews.map(r=>[r.source_id,r])); const reviewBySource=Object.fromEntries(reviews.map(r=>[r.source_id,r])); const tableReviewBySource=Object.fromEntries(tableReviews.map(r=>[r.source_id,r])); const surchargeReviewBySource=Object.fromEntries(surchargeReviews.map(r=>[r.source_id,r])); const list=node("div","","supplier-price-list");
  const scopeLabel={general_cargo:"General cargo",special_cargo:"Özel kargo",mixed:"Karma",unknown:"Belirsiz"}; const reviewLabel={pending:"İnceleme bekliyor",partially_reviewed:"Kısmen incelendi",completed:"İnceleme tamamlandı",no_candidates:"Yapısal aday bulunamadı"};
  sources.forEach(item=>{const validityReview=validityReviewBySource[item.source_id];const review=reviewBySource[item.source_id];const tableReview=tableReviewBySource[item.source_id];const surchargeReview=surchargeReviewBySource[item.source_id];const card=node("div","","supplier-commercial supplier-price-card");const validity=(item.valid_from||item.valid_to)?`${item.valid_from||"?"} → ${item.valid_to||"?"}`:"Geçerlilik belirtilmedi";card.append(node("strong",`${item.airline_name} · ${item.document_name}`),node("span",item.document_stored?"PDF saklandı":"PDF eksik",`badge ${item.document_stored?"open":""}`),node("span",`${scopeLabel[item.cargo_scope]||item.cargo_scope} · Çıkış ${item.origin_airport||"genel"}`,"small"),node("span",`Source metadata validity: ${validity}`,"small"),node("span",`Yüklendi: ${formatDate(item.recorded_at)} · ${item.recorded_by}`,"small muted"));if(validityReview){card.append(node("span",`Tarife geçerliliği doğrulandı: ${validityReview.valid_from} → ${validityReview.valid_to} · ${validityReview.reviewed_by} · ${validityReview.review_note}`,"small muted"));}else{const vf=inboxField("Doğrulanmış başlangıç","date");const vt=inboxField("Doğrulanmış bitiş","date");vf.input.value=item.valid_from||"";vt.input.value=item.valid_to||"";const vn=node("label","Geçerlilik inceleme notu");const vi=document.createElement("input");vi.placeholder="Tarife üzerindeki geçerlilik aralığını hangi kanıtla doğruluyorsun?";vn.append(vi);const vfb=node("div","","muted settings-feedback");const vsave=actionButton("Tarife Geçerliliğini Doğrula","",async()=>{if(!vf.input.value||!vt.input.value||!vi.value.trim()){vfb.textContent="Başlangıç, bitiş ve inceleme notu gerekli.";return;}vsave.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/validity-review`,{method:"POST",body:JSON.stringify({valid_from:vf.input.value,valid_to:vt.input.value,review_note:vi.value.trim()})});await loadSettings();}catch(e){vfb.textContent=e.message||String(e);vsave.disabled=false;}});const vg=node("div","","settings-two-col");vg.append(vf.label,vt.label);card.append(node("span","Bu review yalnız exact source tarih aralığı evidence'ıdır; capacity/schedule veya müşteri fiyatı teyidi değildir.","small muted"),vg,vn,vsave,vfb);}if(review){card.append(node("span",`${reviewLabel[review.status]||review.status} · ${review.candidates?.length||0} yapı adayı`,`badge ${review.status==="completed"?"open":""}`));}else if(item.document_stored){const extract=actionButton("Yapıyı Çıkar","",async()=>{extract.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/extract-structure`,{method:"POST"});await loadSettings();}catch(e){showError(e);extract.disabled=false;}});card.append(extract);}if(tableReview){card.append(node("span",`Satır incelemesi: ${reviewLabel[tableReview.status]||tableReview.status} · ${tableReview.candidates?.length||0} aday`,`badge ${tableReview.status==="completed"?"open":""}`));}else if(review?.status==="completed"){const extractRows=actionButton("Tarife Satırlarını Çıkar","",async()=>{extractRows.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/extract-table`,{method:"POST"});await loadSettings();}catch(e){showError(e);extractRows.disabled=false;}});card.append(extractRows);}if(surchargeReview){card.append(node("span",`Surcharge incelemesi: ${reviewLabel[surchargeReview.status]||surchargeReview.status} · ${surchargeReview.candidates?.length||0} aday`,`badge ${surchargeReview.status==="completed"?"open":""}`));}else if(review?.status==="completed"&&(review.candidates||[]).some(c=>c.kind==="surcharge_label"&&c.status==="confirmed")){const extractSurcharges=actionButton("Surcharge Tutarlarını Çıkar","",async()=>{extractSurcharges.disabled=true;try{await api(`/air-rate-sources/${encodeURIComponent(item.source_id)}/extract-surcharges`,{method:"POST"});await loadSettings();}catch(e){showError(e);extractSurcharges.disabled=false;}});card.append(extractSurcharges);}appendAirWeightRoundingReview(card,item,roundingBySource[item.source_id]||null);appendAirCostScopeReview(card,item,costScopeBySource[item.source_id]||[]);appendAirUnsupportedCostSemanticsReview(card,item,unsupportedCostBySource[item.source_id]||[]);appendAirAdditionalCostEvidence(card,item,additionalCostBySource[item.source_id]||[]);appendAirFxEvidence(card,item,fxBySource[item.source_id]||[]);appendAirAvailabilityEvidence(card,item,availabilityBySource[item.source_id]||[]);list.append(card);});
  if(!sources.length)list.append(emptyState("Henüz ticari havayolu tarife PDF'si yüklenmedi")); panel.append(list);

  const reviewSection=node("div","","approval-focused supplier-price-entry"); reviewSection.append(node("h3","Tarife Yapısı İncelemeleri"),node("div","MIN, +45/+100 gibi weight break etiketleri, surcharge adları, para birimi ve volumetrik divisor adayları burada incelenir. Onaylanan adaylar bile fiyat authority'si değildir.","small muted"));
  const kindLabel={weight_break:"Weight break",surcharge_label:"Surcharge etiketi",currency:"Para birimi",volumetric_divisor:"Volumetrik bölen",cargo_scope_hint:"Kargo kapsamı ipucu"};
  reviews.forEach(review=>{const box=node("div","","supplier-commercial supplier-price-card");box.append(node("strong",`${review.airline_name||"Havayolu"} · ${review.document_name||review.source_id}`),node("span",reviewLabel[review.status]||review.status,"badge"),node("span",`${review.extracted_character_count||0} karakter · AI parser: hayır`,"small muted"));(review.candidates||[]).forEach(candidate=>{const row=node("div","","supplier-commercial");row.append(node("strong",`${kindLabel[candidate.kind]||candidate.kind}: ${candidate.value}`),node("span",`${candidate.occurrence_count} gözlem · satır ${(candidate.source_line_numbers||[]).join(", ")||"-"}`,"small muted"),node("span",candidate.status,"badge"));if(candidate.status==="proposed"){const noteLabel=node("label","İnceleme notu");const note=document.createElement("input");note.placeholder="Neden doğru / yanlış?";noteLabel.append(note);const fb=node("div","","muted settings-feedback");const decide=async(decision,button)=>{if(!note.value.trim()){fb.textContent="İnceleme notu gerekli.";return;}button.disabled=true;try{await api(`/air-rate-structure-reviews/${encodeURIComponent(review.review_id)}/candidates/${encodeURIComponent(candidate.candidate_id)}/decision`,{method:"POST",body:JSON.stringify({decision,review_note:note.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);button.disabled=false;}};const actions=node("div","","actions");const confirm=actionButton("Doğrula","approve",()=>decide("confirm",confirm));const reject=actionButton("Reddet","reject",()=>decide("reject",reject));actions.append(confirm,reject);row.append(noteLabel,actions,fb);}else{row.append(node("span",`${candidate.reviewed_by||"-"} · ${candidate.review_note||"-"}`,"small muted"));}box.append(row);});if(!(review.candidates||[]).length)box.append(node("div","Bu PDF'de v1 yapısal etiketleri bulunamadı. Tam tarife yorumu yapılmadı.","small muted"));reviewSection.append(box);});
  if(!reviews.length)reviewSection.append(emptyState("Henüz tarife yapısı çıkarılmadı"));panel.append(reviewSection);

  const tableSection=node("div","","approval-focused supplier-price-entry");tableSection.append(node("h3","Tarife Satırı İncelemeleri"),node("div","Destination + doğrulanmış weight-break kolonlarından deterministik çıkarılan sayısal satırlar burada incelenir. Doğrulanan satırlar yalnız referans verisidir; quote hesabına, pivot-weight hesabına veya müşteri fiyatına bağlanmaz.","small muted"));
  tableReviews.forEach(review=>{const box=node("div","","supplier-commercial supplier-price-card");box.append(node("strong",`${review.airline_name||"Havayolu"} · ${review.document_name||review.source_id}`),node("span",reviewLabel[review.status]||review.status,"badge"),node("span",`${(review.weight_breaks||[]).join(" / ")} · AI parser: hayır`,`small muted`));(review.candidates||[]).forEach(candidate=>{const row=node("div","","supplier-commercial");const rateText=Object.entries(candidate.rates||{}).map(([k,v])=>`${k}: ${v}`).join(" · ");row.append(node("strong",`${candidate.destination_label}${candidate.destination_code&&candidate.destination_code!==candidate.destination_label?` (${candidate.destination_code})`:""} · ${candidate.currency||"para birimi belirsiz"}`),node("span",rateText||"Fiyat kolonları yok","small"),node("span",`Kaynak satır ${candidate.source_line_number} · ${candidate.status}`,"small muted"));if(candidate.status==="proposed"){const noteLabel=node("label","Satır inceleme notu");const note=document.createElement("input");note.placeholder="Satır ve kolon hizalaması doğru mu?";noteLabel.append(note);const fb=node("div","","muted settings-feedback");const decide=async(decision,button)=>{if(!note.value.trim()){fb.textContent="İnceleme notu gerekli.";return;}button.disabled=true;try{await api(`/air-rate-table-reviews/${encodeURIComponent(review.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/decision`,{method:"POST",body:JSON.stringify({decision,review_note:note.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);button.disabled=false;}};const actions=node("div","","actions");const confirm=actionButton("Satırı Doğrula","approve",()=>decide("confirm",confirm));const reject=actionButton("Satırı Reddet","reject",()=>decide("reject",reject));actions.append(confirm,reject);row.append(noteLabel,actions,fb);}else{row.append(node("span",`${candidate.reviewed_by||"-"} · ${candidate.review_note||"-"}`,"small muted"));if(candidate.status==="confirmed"){const previewBox=node("div","","supplier-commercial");previewBox.append(node("strong","Navlun Önizleme"),node("span","Surcharge dahil değil · Müşteri teklifi değildir · Capacity teyidi değildir · Airline kg rounding yalnız exact-source review varsa uygulanır","small muted"));const actual=numberField("Gerçek ağırlık (kg)",100,0.01,1000000);const volumetric=numberField("Hacimsel ağırlık (kg)",100,0.01,1000000);const result=node("div","","small muted");const run=actionButton("Pivot Hesapla","",async()=>{run.disabled=true;result.textContent="Hesaplanıyor…";try{const out=await api(`/air-rate-table-reviews/${encodeURIComponent(review.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/calculation-preview`,{method:"POST",body:JSON.stringify({actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value()})});const options=(out.options||[]).map(o=>`${o.weight_break}: ${o.billed_weight_kg} kg × ${o.rate_per_kg} = ${o.base_freight} ${out.currency}${o.minimum_applied?" (MIN)":""}`).join(" | ");const rounding=out.rounding_review_id?(out.rounding_mode==="none"?"reviewed: none":`reviewed ceiling ${out.rounding_increment_kg} kg → ${out.rounded_chargeable_weight_kg} kg`):"unreviewed / raw chargeable";result.textContent=`Chargeable ${out.chargeable_weight_kg} kg · Weight rounding ${rounding} · Önerilen ${out.recommended_break} / ${out.recommended_billed_weight_kg} kg · Base freight ${out.recommended_base_freight} ${out.currency}${out.pivot_applied?" · PIVOT":""} · ${options}`;}catch(e){result.textContent=e.message||String(e);}finally{run.disabled=false;}});const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label);previewBox.append(grid,run,result);row.append(previewBox);appendReviewedSurchargeCostPreview(row,review,candidate,surchargeReviewBySource[review.source_id],additionalCostBySource[review.source_id]||[],costScopeBySource[review.source_id]||[],unsupportedCostBySource[review.source_id]||[],fxBySource[review.source_id]||[],customerProfiles,minaJobs);}}box.append(row);});if(!(review.candidates||[]).length)box.append(node("div","v1'in güvenle hizalayabildiği numeric tarife satırı bulunamadı; manuel tahmin yapılmadı.","small muted"));tableSection.append(box);});
  if(!tableReviews.length)tableSection.append(emptyState("Henüz satır seviyesi tarife incelemesi yok"));panel.append(tableSection);

  const surchargeSection=node("div","","approval-focused supplier-price-entry");surchargeSection.append(node("h3","Surcharge Tutar İncelemeleri"),node("div","Yalnız açık para birimi + tutar + /kg veya /shipment gibi unit içeren satırlar adaylaştırılır. Unit belirsiz ve yüzde bazlı satırlar tahmin edilmez. Base Navlun Önizleme surcharge içermez; yalnız ayrı Reviewed Surcharge Cost Preview tüm review zinciri tamamlanmış uygun per-kg kalemleri ve reviewed quantity basis + açık count bulunan flat kalemleri partial reference cost olarak tüketebilir.","small muted"));
  ...
  surchargeReviews.forEach(review=>{
    ...
    box.append(node("strong",`${review.airline_name||"Havayolu"} · ${review.document_name||review.source_id}`),node("span",reviewLabel[review.status]||review.status,"badge"),node("span","AI parser: hayır · doğrudan pricing authority yok · yalnız ayrı reviewed preview kontrollü tüketebilir","small muted"));
      ...
      if(candidate.status==="proposed"){
        ...
        if(candidate.status==="confirmed"){
          if(candidate.application_basis){
            ...
            if(candidate.applicability_scope){
              const scopeText=candidate.applicability_scope==="source_wide"?"Bu PDF kaynağının kapsadığı destinasyonlar":"Destination specific";
              ...
              if(candidate.cargo_applicability){
                ...
                if(candidate.basis==="flat"){
                  if(candidate.flat_quantity_basis){
                    ...
                    row.append(node("span","Bu review yalnız quantity semantics evidence'ıdır; matching explicit count verilmeden flat ücret cost preview'a eklenmez.","small muted"),qLabel,qNoteLabel,qSave,qFb);
                  }
                }
              }else{
                ...
                const viaLabel=node("label","Via airport (gerekiyorsa)");const via=document.createElement("input");via.maxLength=3;via.placeholder="IST";viaLabel.append(via);const noteLabel=node("label","Koşul inceleme notu");const note=document.createElement("input");note.placeholder="Cargo/routing koşulunu hangi kanıtla doğruluyorsun?";noteLabel.append(note);const fb=node("div","","muted settings-feedback");const save=actionButton("Operasyon Koşullarını Doğrula","",async()=>{if(!note.value.trim()){fb.textContent="İnceleme notu gerekli.";return;}const viaCode=via.value.trim().toUpperCase();if(routing.value==="via_airport"&&!/^[A-Z]{3}$/.test(viaCode)){fb.textContent="Via airport için 3 harfli kod gerekli.";return;}save.disabled=true;try{await api(`/air-rate-surcharge-reviews/${encodeURIComponent(review.review_id)}/candidates/${encodeURIComponent(candidate.candidate_id)}/operational-conditions`,{method:"POST",body:JSON.stringify({cargo_applicability:cargo.value,routing_applicability:routing.value,via_airport:routing.value==="via_airport"?viaCode:null,review_note:note.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);save.disabled=false;}});row.append(node("span","Bu karar tek başına surcharge hesap tüketimini açmaz; tüm review zinciri ve preview bağlamı ayrıca eşleşmelidir.","small muted"),cargoLabel,routingLabel,viaLabel,noteLabel,save,fb);
              }
            }else{
              const scopeLabel=node("label","Surcharge uygulanabilirlik kapsamı");const scope=document.createElement("select");[["source_wide","Bu PDF kaynağının kapsadığı destinasyonlar"],["destination_specific","Belirli destination"]].forEach(([v,t])=>{const option=document.createElement("option");option.value=v;option.textContent=t;scope.append(option);});scopeLabel.append(scope);
              const destinationLabel=node("label","Destination code (yalnız belirli destination)");const destination=document.createElement("input");destination.maxLength=3;destination.placeholder="FRA";destinationLabel.append(destination);
              const scopeNoteLabel=node("label","Uygulanabilirlik inceleme notu");const scopeNote=document.createElement("input");scopeNote.placeholder="Bu surcharge'ın hangi destinasyonlara uygulandığını hangi kanıtla doğruluyorsun?";scopeNoteLabel.append(scopeNote);
              const scopeFb=node("div","","muted settings-feedback");const saveScope=actionButton("Uygulanabilirlik Kapsamını Doğrula","",async()=>{if(!scopeNote.value.trim()){scopeFb.textContent="İnceleme notu gerekli.";return;}const code=destination.value.trim().toUpperCase();if(scope.value==="destination_specific"&&!/^[A-Z]{3}$/.test(code)){scopeFb.textContent="Belirli destination için 3 harfli kod gerekli.";return;}saveScope.disabled=true;try{await api(`/air-rate-surcharge-reviews/${encodeURIComponent(review.review_id)}/candidates/${encodeURIComponent(candidate.candidate_id)}/applicability-scope`,{method:"POST",body:JSON.stringify({applicability_scope:scope.value,destination_code:scope.value==="destination_specific"?code:null,review_note:scopeNote.value.trim()})});await loadSettings();}catch(e){scopeFb.textContent=e.message||String(e);saveScope.disabled=false;}});row.append(node("span","Bu doğrulama da surcharge'ı hesaba katmaz; routing ve özel-kargo koşulları ayrıca çözülmelidir.","small muted"),scopeLabel,destinationLabel,scopeNoteLabel,saveScope,scopeFb);
            }
          }else{
  ...
  const airline=inboxField("Havayolu");const airport=inboxField("Çıkış havalimanı (opsiyonel)");airport.input.maxLength=3;airport.input.placeholder="ADA / IST";
  const scopeLabelNode=node("label","Kargo kapsamı");const scope=document.createElement("select");[["unknown","Belirsiz"],["general_cargo","General cargo"],["special_cargo","Özel kargo"],["mixed","Karma"]].forEach(([v,t])=>{const o=document.createElement("option");o.value=v;o.textContent=t;scope.append(o);});scopeLabelNode.append(scope);
  const validFrom=inboxField("Geçerli başlangıç","date");const validTo=inboxField("Geçerli bitiş","date");
  const fileLabel=node("label","Tarife PDF");const file=document.createElement("input");file.type="file";file.accept="application/pdf,.pdf";fileLabel.append(file);
  const notesLabel=node("label","Not");const notes=document.createElement("textarea");notes.rows=3;notes.placeholder="Örn. airline tarafından gönderilen Eylül general cargo tarifesi";notesLabel.append(notes);
  const fb=node("div","","muted settings-feedback");const save=actionButton("PDF Listeyi Kaydet","primary",async()=>{const selected=file.files?.[0];if(!airline.input.value.trim()||!selected){fb.textContent="Havayolu ve PDF dosyası gerekli.";return;}if(!selected.name.toLowerCase().endsWith(".pdf")){fb.textContent="Yalnız PDF kabul edilir.";return;}if(selected.size>10*1024*1024){fb.textContent="PDF 10 MiB sınırını aşıyor.";return;}const params=new URLSearchParams({entry_id:freshPriceEntryId("web-air-rate-source"),airline_name:airline.input.value.trim(),document_name:selected.name,cargo_scope:scope.value});const ap=airport.input.value.trim().toUpperCase();if(ap)params.set("origin_airport",ap);if(validFrom.input.value)params.set("valid_from",validFrom.input.value);if(validTo.input.value)params.set("valid_to",validTo.input.value);if(notes.value.trim())params.set("notes",notes.value.trim());save.disabled=true;fb.textContent="PDF güvenli depoya kaydediliyor…";try{await api(`/air-rate-sources/upload?${params.toString()}`,{method:"POST",headers:{"Content-Type":"application/pdf"},body:selected});fb.textContent="Tarife kaynağı kaydedildi; henüz fiyat authority'si değildir.";await loadSettings();}catch(e){fb.textContent=e.message||String(e);}finally{save.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(airline.label,airport.label,scopeLabelNode,validFrom.label,validTo.label,fileLabel);form.append(grid,notesLabel,save,fb);panel.append(form);return panel;
}
...
function renderPasswordSettings(status={}) {
  const panel=node("section","","settings-panel");panel.append(node("h2","Şifre Değiştir"));
  if(!status.available){panel.append(node("div","Şifre değiştirme bu ortamda kullanılamıyor. Deployment sahibi MINAI_WEB_PASSWORD_OVERRIDES_PATH ayarını güvenli harici bir dosyaya yönlendirmelidir.","warning"));return panel;}
  panel.append(node("p","Mevcut şifreni doğrula. Yeni şifre 10–128 karakter olmalı; özel karakter veya büyük/küçük harf zorunluluğu yoktur. Başarılı değişiklik tüm oturumlarını kapatır.","muted"));
  const currentLabel=node("label","Mevcut şifre");const current=document.createElement("input");current.type="password";current.autocomplete="current-password";current.maxLength=128;currentLabel.append(current);
  const nextLabel=node("label","Yeni şifre");const next=document.createElement("input");next.type="password";next.autocomplete="new-password";next.minLength=10;next.maxLength=128;nextLabel.append(next);
  const repeatLabel=node("label","Yeni şifre tekrar");const repeat=document.createElement("input");repeat.type="password";repeat.autocomplete="new-password";repeat.minLength=10;repeat.maxLength=128;repeatLabel.append(repeat);
  const fb=node("div","","settings-feedback");const save=actionButton("Şifremi Değiştir","primary",async()=>{if(next.value!==repeat.value){fb.textContent="Yeni şifreler eşleşmiyor.";return;}if(next.value.length<10||next.value.length>128){fb.textContent="Yeni şifre 10–128 karakter olmalı.";return;}save.disabled=true;try{await api("/settings/password/change",{method:"POST",body:JSON.stringify({current_password:current.value,new_password:next.value,new_password_confirmation:repeat.value})});current.value=next.value=repeat.value="";window.location.assign("/app/login");}catch(e){current.value="";fb.textContent=e.message||String(e);save.disabled=false;}});panel.append(currentLabel,nextLabel,repeatLabel,save,fb);return panel;
}

let settingsSelectedTab="automation";
function renderSettings(branding, automationPolicy, customersPayload = {}, suppliersPayload = {}, performanceSettings = {}, mailboxStatus = {}, relationshipStatus = {}, fixedRates = {}, airRates = {}, airRateValidity = {}, airRateRounding = {}, airAdditionalCosts = {}, airCostScopes = {}, airUnsupportedCosts = {}, airFx = {}, airAvailability = {}, airRateReviews = {}, airRateTables = {}, airRateSurcharges = {}, minaJobsPayload = {}, systemHealth = {}, customerMemoryBundle = null, runtimeCapabilities = {}, passwordStatus = {}) {
