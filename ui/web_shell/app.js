...
function appendReviewedSurchargeCostPreview(row, tableReview, candidate, surchargeReview) {
  ...
  const run=actionButton("Reviewed Surcharge Cost Hesapla","",async()=>{run.disabled=true;result.textContent="Hesaplanıyor…";try{const body={actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/reviewed-surcharge-cost-preview`,{method:"POST",body:JSON.stringify(body)});const perKg=(out.included_surcharges||[]).map(x=>`${x.surcharge_code}: ${x.applied_weight_kg} kg × ${x.rate_per_kg} = ${x.surcharge_cost} ${x.currency} (${x.application_basis})`).join(" | ")||"Dahil edilen per-kg surcharge yok";const flat=(out.included_flat_surcharges||[]).map(x=>`${x.surcharge_code}: ${x.applied_count} × ${x.amount_per_unit} = ${x.surcharge_cost} ${x.currency} (${x.quantity_basis})`).join(" | ")||"Dahil edilen flat surcharge yok";const excluded=(out.excluded_surcharges||[]).map(x=>`${x.surcharge_code}/${x.currency}: ${x.reason}`).join(" | ")||"Hariç kalem yok";result.textContent=`Base freight ${out.freight.recommended_base_freight} ${out.freight.currency} · Reviewed per-kg ${out.reviewed_per_kg_surcharge_total} · Reviewed flat ${out.reviewed_flat_surcharge_total} · Base + reviewed surcharge ${out.base_plus_reviewed_surcharges} ${out.freight.currency} · ALL-IN DEĞİL · Per-kg: ${perKg} · Flat: ${flat} · Hariç: ${excluded}`;}catch(e){result.textContent=e.message||String(e);}finally{run.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label,cargoLabel,routingLabel,viaLabel,shipmentCount.label,awbCount.label,hawbCount.label,mawbCount.label);box.append(grid,node("span","Count alanları boş bırakılırsa hiçbir count varsayılmaz; uygulanabilir fully-reviewed flat kalem gerekli count olmadan fail-closed olur.","small muted"),run,result);row.append(box);
}

function renderAirRateSourceSettings(payload = {}, reviewsPayload = {}, tablePayload = {}, surchargePayload = {}) {
  const panel=node("section","","settings-panel"); const h=node("div","","settings-heading");
  ...
  const sources=(payload.rate_sources||[]).slice(); const reviews=(reviewsPayload.reviews||[]).slice(); const tableReviews=(tablePayload.reviews||[]).slice(); const surchargeReviews=(surchargePayload.reviews||[]).slice(); const reviewBySource=Object.fromEntries(reviews.map(r=>[r.source_id,r])); const tableReviewBySource=Object.fromEntries(tableReviews.map(r=>[r.source_id,r])); const surchargeReviewBySource=Object.fromEntries(surchargeReviews.map(r=>[r.source_id,r])); const list=node("div","","supplier-price-list");
...
function renderSettings(branding, automationPolicy, customersPayload = {}, suppliersPayload = {}, performanceSettings = {}, relationshipStatus = {}, fixedRates = {}, airRates = {}, airRateReviews = {}, airRateTables = {}, airRateSurcharges = {}, systemHealth = {}, customerMemoryBundle = null) {
...
async function loadSettings() {
  const [branding, automationPolicy, customersPayload, suppliersPayload, performanceSettings, relationshipStatus, fixedRates, airRates, airRateReviews, airRateTables, airRateSurcharges, runtime, automation, dataHealth, commodityValidation, supplierValidation, customerMemoryValidation, hsValidation] = await Promise.all([
    api("/settings/branding"), api("/automation-policy/agency"), api("/master-data/customers"), api("/master-data/suppliers"), api("/settings/performance"), api("/relationship-onboarding/status"), api("/supplier-fixed-rates"), api("/air-rate-sources"), api("/air-rate-structure-reviews"), api("/air-rate-table-reviews"), api("/air-rate-surcharge-reviews"), api("/runtime/release"), api("/automation/status"), api("/data-health/summary"), api("/commodity-dictionary/validation"), api("/supplier-capabilities/validation"), api("/customer-memory/validation"), api("/hs-commodity-map/validation")
  ]);
