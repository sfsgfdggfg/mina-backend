...
function renderAirRateSourceSettings(payload = {}, reviewsPayload = {}, tablePayload = {}) {
  const panel=node("section","","settings-panel"); const h=node("div","","settings-heading");
  h.append(node("h2","Havayolu Listeleri"),node("p","Ticari havayolu tarife PDF'lerini shadow-learning için kaydeder; yapısal etiketleri ve insan incelemesine tabi numeric satır adaylarını yerelde çıkarır. Quote üretmez ve express tarifesi kabul etmez.","muted")); panel.append(h);
  panel.append(node("div","PDF ticari sır olarak korunur. Bu ekran fiyat üretmez. Yapı ve satır çıkarımı yerel/deterministik çalışır; AI parser çağrılmaz ve tam extracted text state/audit kaydına yazılmaz.","notice"));
  const sources=(payload.rate_sources||[]).slice(); const reviews=(reviewsPayload.reviews||[]).slice(); const tableReviews=(tablePayload.reviews||[]).slice(); const reviewBySource=Object.fromEntries(reviews.map(r=>[r.source_id,r])); const tableReviewBySource=Object.fromEntries(tableReviews.map(r=>[r.source_id,r])); const list=node("div","","supplier-price-list");
  ...
  tableReviews.forEach(review=>{const box=node("div","","supplier-commercial supplier-price-card");box.append(node("strong",`${review.airline_name||"Havayolu"} · ${review.document_name||review.source_id}`),node("span",reviewLabel[review.status]||review.status,"badge"),node("span",`${(review.weight_breaks||[]).join(" / ")} · AI parser: hayır`,`small muted`));(review.candidates||[]).forEach(candidate=>{const row=node("div","","supplier-commercial");const rateText=Object.entries(candidate.rates||{}).map(([k,v])=>`${k}: ${v}`).join(" · ");row.append(node("strong",`${candidate.destination_label}${candidate.destination_code&&candidate.destination_code!==candidate.destination_label?` (${candidate.destination_code})`:""} · ${candidate.currency||"para birimi belirsiz"}`),node("span",rateText||"Fiyat kolonları yok","small"),node("span",`Kaynak satır ${candidate.source_line_number} · ${candidate.status}`,"small muted"));if(candidate.status==="proposed"){const noteLabel=node("label","Satır inceleme notu");const note=document.createElement("input");note.placeholder="Satır ve kolon hizalaması doğru mu?";noteLabel.append(note);const fb=node("div","","muted settings-feedback");const decide=async(decision,button)=>{if(!note.value.trim()){fb.textContent="İnceleme notu gerekli.";return;}button.disabled=true;try{await api(`/air-rate-table-reviews/${encodeURIComponent(review.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/decision`,{method:"POST",body:JSON.stringify({decision,review_note:note.value.trim()})});await loadSettings();}catch(e){fb.textContent=e.message||String(e);button.disabled=false;}};const actions=node("div","","actions");const confirm=actionButton("Satırı Doğrula","approve",()=>decide("confirm",confirm));const reject=actionButton("Satırı Reddet","reject",()=>decide("reject",reject));actions.append(confirm,reject);row.append(noteLabel,actions,fb);}else{row.append(node("span",`${candidate.reviewed_by||"-"} · ${candidate.review_note||"-"}`,"small muted"));if(candidate.status==="confirmed"){const calc=node("div","","supplier-commercial");const weightLabel=node("label","Gerçek ağırlık (kg)");const weight=document.createElement("input");weight.type="number";weight.min="0.001";weight.step="0.001";weightLabel.append(weight);const packagesLabel=node("label","Paketler · her satır: adet, boy, en, yükseklik (cm)");const packages=document.createElement("textarea");packages.rows=3;packages.placeholder="2,120,80,100\n1,100,100,90";packagesLabel.append(packages);const result=node("div","Surcharge dahil değil · kg rounding uygulanmadı · capacity/schedule teyidi değildir · müşteri quote'u değildir.","small muted");const calculate=actionButton("Shadow Hesap","",async()=>{const actual=Number(weight.value);const parsed=[];for(const line of packages.value.split(/\n+/).map(x=>x.trim()).filter(Boolean)){const parts=line.split(/[xX,;\s]+/).filter(Boolean).map(Number);if(parts.length!==4||parts.some(x=>!Number.isFinite(x)||x<=0)){result.textContent="Paket formatı geçersiz. Örnek: 2,120,80,100";return;}parsed.push({quantity:parts[0],length_cm:parts[1],width_cm:parts[2],height_cm:parts[3]});}if(!Number.isFinite(actual)||actual<=0||!parsed.length){result.textContent="Gerçek ağırlık ve en az bir paket satırı gerekli.";return;}calculate.disabled=true;try{const response=await api(`/air-rate-table-reviews/${encodeURIComponent(review.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/shadow-preview`,{method:"POST",body:JSON.stringify({actual_weight_kg:actual,packages:parsed})});const p=response.preview;const pivotText=(p.pivot_options||[]).map(x=>`${x.rate_break}→${x.billed_weight_kg} kg / ${x.freight_amount} ${x.currency}`).join(" · ")||"yok";result.textContent=`Chargeable ${p.chargeable_weight_kg_unrounded} kg (actual ${p.actual_weight_kg} / volumetric ${p.volumetric_weight_kg}, divisor ${p.divisor}) · Standart ${p.standard_option.rate_break}: ${p.standard_option.freight_amount} ${p.currency} · Pivot: ${pivotText} · Matematiksel en düşük ${p.lowest_math_option.rate_break}: ${p.lowest_math_option.freight_amount} ${p.currency}. Surcharge dahil değil · kg rounding uygulanmadı · capacity/schedule teyidi değildir · müşteri quote'u değildir.`;}catch(e){result.textContent=e.message||String(e);}finally{calculate.disabled=false;}});calc.append(weightLabel,packagesLabel,calculate,result);row.append(calc);}}box.append(row);});if(!(review.candidates||[]).length)box.append(node("div","v1'in güvenle hizalayabildiği numeric tarife satırı bulunamadı; manuel tahmin yapılmadı.","small muted"));tableSection.append(box);});
...
function renderSettings(branding, automationPolicy, customersPayload = {}, suppliersPayload = {}, performanceSettings = {}, relationshipStatus = {}, fixedRates = {}, airRates = {}, airRateReviews = {}, airRateTables = {}, systemHealth = {}, customerMemoryBundle = null) {
...
async function loadSettings() {
  const [branding, automationPolicy, customersPayload, suppliersPayload, performanceSettings, relationshipStatus, fixedRates, airRates, airRateReviews, airRateTables, runtime, automation, dataHealth, commodityValidation, supplierValidation, customerMemoryValidation, hsValidation] = await Promise.all([
    api("/settings/branding"), api("/automation-policy/agency"), api("/master-data/customers"), api("/master-data/suppliers"), api("/settings/performance"), api("/relationship-onboarding/status"), api("/supplier-fixed-rates"), api("/air-rate-sources"), api("/air-rate-structure-reviews"), api("/air-rate-table-reviews"), api("/runtime/release"), api("/automation/status"), api("/data-health/summary"), api("/commodity-dictionary/validation"), api("/supplier-capabilities/validation"), api("/customer-memory/validation"), api("/hs-commodity-map/validation")
  ]);
