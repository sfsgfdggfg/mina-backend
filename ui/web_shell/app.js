...
function appendReviewedSurchargeCostPreview(row, tableReview, candidate, surchargeReview) {
  if(!surchargeReview || surchargeReview.status!=="completed") return;
  const box=node("div","","supplier-commercial");
  box.append(node("strong","Reviewed Surcharge Cost Preview"),node("span","Aynı currency + eşleşen human-review evidence. Per-kg kalemler reviewed weight basis ile; flat kalemler yalnız reviewed quantity basis + açık count ile hesaplanır. FX hariç · All-in değildir · Müşteri teklifi değildir · Capacity/booking teyidi değildir; tariff validity yalnız explicit reference date + reviewed validity ile doğrulanır.","small muted"));
  ...
  const run=actionButton("Reviewed Surcharge Cost Hesapla","",async()=>{run.disabled=true;result.textContent="Hesaplanıyor…";try{const body={actual_weight_kg:actual.value(),volumetric_weight_kg:volumetric.value(),cargo_context:cargo.value,routing_context:routing.value};const viaCode=via.value.trim().toUpperCase();if(viaCode)body.via_airport=viaCode;if(referenceDate.input.value)body.reference_date=referenceDate.input.value;addCount(body,"shipment_count",shipmentCount);addCount(body,"awb_count",awbCount);addCount(body,"hawb_count",hawbCount);addCount(body,"mawb_count",mawbCount);const out=await api(`/air-rate-table-reviews/${encodeURIComponent(tableReview.review_id)}/rows/${encodeURIComponent(candidate.candidate_id)}/reviewed-surcharge-cost-preview`,{method:"POST",body:JSON.stringify(body)});const perKg=(out.included_surcharges||[]).map(x=>`${x.surcharge_code}: ${x.applied_weight_kg} kg × ${x.rate_per_kg} = ${x.surcharge_cost} ${x.currency} (${x.application_basis})`).join(" | ")||"Dahil edilen per-kg surcharge yok";const flat=(out.included_flat_surcharges||[]).map(x=>`${x.surcharge_code}: ${x.applied_count} × ${x.amount_per_unit} = ${x.surcharge_cost} ${x.currency} (${x.quantity_basis})`).join(" | ")||"Dahil edilen flat surcharge yok";const excluded=(out.excluded_surcharges||[]).map(x=>`${x.surcharge_code}/${x.currency}: ${x.reason}`).join(" | ")||"Hariç kalem yok";result.textContent=`Base freight ${out.freight.recommended_base_freight} ${out.freight.currency} · Reviewed per-kg ${out.reviewed_per_kg_surcharge_total} · Reviewed flat ${out.reviewed_flat_surcharge_total} · Base + reviewed surcharge ${out.base_plus_reviewed_surcharges} ${out.freight.currency} · Tarife geçerliliği ${out.tariff_validity_confirmed?`DOĞRULANDI (${out.reviewed_valid_from} → ${out.reviewed_valid_to}; ref ${out.reference_date})`:"DOĞRULANMADI"} · ALL-IN DEĞİL · Per-kg: ${perKg} · Flat: ${flat} · Hariç: ${excluded}`;}catch(e){result.textContent=e.message||String(e);}finally{run.disabled=false;}});
  const grid=node("div","","settings-two-col");grid.append(actual.label,volumetric.label,cargoLabel,routingLabel,viaLabel,referenceDate.label,shipmentCount.label,awbCount.label,hawbCount.label,mawbCount.label);box.append(grid,node("span","Tarife referans tarihi boşsa bugünün tarihi varsayılmaz ve validity teyidi oluşmaz. Count alanları boş bırakılırsa hiçbir count varsayılmaz; uygulanabilir fully-reviewed flat kalem gerekli count olmadan fail-closed olur.","small muted"),run,result);row.append(box);
}

function appendAirFxEvidence(card, item, evidence = []) {
  const box=node("div","","supplier-commercial");
  box.append(node("strong","FX Rate Evidence"),node("span","Yalnız inquiry + exact tariff source evidence'ıdır. Kur yönü: 1 BASE = rate QUOTE. Cost preview bu kaydı henüz tüketmez.","small muted"));
