...
class AirFxRateEvidenceConflictError(ValueError):
    pass


def _creation_payload(item: AirFxRateEvidence) -> dict:
    return item.model_dump(mode="json", exclude={"evidence_id", "recorded_at"})


class AirFxRateEvidenceRepository(Protocol):
    def create(self, item: AirFxRateEvidence) -> tuple[AirFxRateEvidence, bool]: ...
    def get(self, evidence_id: str) -> AirFxRateEvidence | None: ...
    def find_by_entry(self, entry_id: str) -> AirFxRateEvidence | None: ...
    def list_all(self) -> list[AirFxRateEvidence]: ...


class InMemoryAirFxRateEvidenceRepository:
    def __init__(self) -> None:
        self.items: dict[str, AirFxRateEvidence] = {}
        ...
        with self._lock:
            ...
            if existing is not None:
                if _creation_payload(existing) != _creation_payload(item):
                    raise AirFxRateEvidenceConflictError(
                        "Air FX evidence entry_id reused with different evidence."
                    )
...
class SQLiteAirFxRateEvidenceRepository:
    ITEM_NS = "air_fx_rate_evidence"
    ENTRY_NS = "air_fx_rate_evidence_by_entry"
    ...
    def create(self, item):
        ...
        with scope:
            if not self.store.insert_once(
                ...
                event_type="air_fx_rate_evidence_created",
                entity_type="air_fx_rate_evidence",
            ):
                raise AirFxRateEvidenceConflictError("Air FX evidence id collision.")
            if not self.store.insert_once(
                namespace=self.ENTRY_NS,
                record_key=item.entry_id,
                payload={"evidence_id": item.evidence_id},
                event_type="air_fx_rate_evidence_indexed",
                entity_type="air_fx_rate_evidence_index",
            ):
                raise AirFxRateEvidenceConflictError("Air FX evidence entry identity collision.")
        return item.model_copy(deep=True), True

    def get(self, evidence_id):
        payload = self.store.get(namespace=self.ITEM_NS, record_key=evidence_id)
        return None if payload is None else AirFxRateEvidence.model_validate(payload)
    ...
    def list_all(self):
        return [
            AirFxRateEvidence.model_validate(item)
