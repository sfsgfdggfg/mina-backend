...
def evaluate_pilot_operator_regressions() -> dict:
    ...
    client.get_operational_shift_close_readiness()
    ...
    contracts.append((_last_contract(session), ("GET", "/operational-work-shift-close-receipts", None)))
    ...
    close_readiness_cli_session = _Session([_Response(200, {"ready_to_close": True})])
    close_readiness_cli_client = _client(close_readiness_cli_session)
    with patch.object(PilotOperatorClient, "from_environment", return_value=close_readiness_cli_client):
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            close_readiness_cli_exit = main(["work", "close-readiness"])
    if close_readiness_cli_exit != 0:
        failures.append("operational shift-close readiness CLI command failed")
    elif _last_contract(close_readiness_cli_session) != (
        "GET", "/operational-work-shift-close-readiness", None,
    ):
        failures.append("operational shift-close readiness CLI mapped to the wrong API contract")

    open_reconciliation_cli_session = _Session([_Response(200, {"reconciliation_status": "clear"})])
    open_reconciliation_cli_client = _client(open_reconciliation_cli_session)
    with patch.object(PilotOperatorClient, "from_environment", return_value=open_reconciliation_cli_client):
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            open_reconciliation_cli_exit = main(["work", "open-reconciliation"])
    if open_reconciliation_cli_exit != 0:
        failures.append("operational shift-open reconciliation CLI command failed")
    elif _last_contract(open_reconciliation_cli_session) != (
    ...
    with patch.object(PilotOperatorClient, "from_environment", return_value=close_attest_cli_client):
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            close_attest_cli_exit = main(["work", "close-attest"])
    if close_attest_cli_exit != 0:
        failures.append("operational shift-close attestation CLI command failed")
    elif _last_contract(close_attest_cli_session) != (
        "POST", "/operational-work-shift-close-attest", {},
    ):
        failures.append("operational shift-close attestation CLI mapped to the wrong API contract")

    close_receipts_cli_session = _Session([_Response(200, {"count": 0, "items": []})])
    close_receipts_cli_client = _client(close_receipts_cli_session)
    with patch.object(PilotOperatorClient, "from_environment", return_value=close_receipts_cli_client):
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            close_receipts_cli_exit = main(["work", "close-receipts"])
    if close_receipts_cli_exit != 0:
        failures.append("operational shift-close receipts CLI command failed")
    elif _last_contract(close_receipts_cli_session) != (
        "GET", "/operational-work-shift-close-receipts", None,
    ):
        failures.append("operational shift-close receipts CLI mapped to the wrong API contract")
