...
from src.core.customer_quote_reason_policy import build_customer_quote_reason_policy
...
class AirRateSurchargeOperationalConditionsRequest(BaseModel):
...
class AirRateSurchargeFlatQuantityBasisRequest(BaseModel):
...
@app.post("/master-data/customers/{customer_id}/derive-quote-reason-learning")
def derive_customer_quote_reasons(customer_id: str, http_request: Request):
    try:
...
@app.get("/master-data/customers/{customer_id}/quote-reason-policy")
def get_customer_quote_reason_policy(customer_id: str):
    customer = master_data_repository.get_customer(customer_id)
    if customer is None:
        raise HTTPException(status_code=404, detail=f"Customer master not found: {customer_id}")
    return build_customer_quote_reason_policy(
        customer_id=customer_id, learning_repository=learning_fact_repository,
        as_of=datetime.now(timezone.utc),
    ).model_dump(mode="json")
...
@app.get("/air-rate-table-reviews")
def list_air_rate_table_reviews():
    return build_air_rate_table_review_view(
        repository=air_rate_table_review_repository,
        source_repository=air_shadow_repository,
    )


@app.post("/air-rate-sources/{source_id}/extract-table")
def extract_air_rate_table(source_id: str, http_request: Request):
    try:
        review, created = create_air_rate_table_review(
            source_id=source_id,
            source_repository=air_shadow_repository,
            structure_repository=air_rate_structure_review_repository,
            review_repository=air_rate_table_review_repository,
            document_store=air_rate_document_store,
            requested_by=_authenticated_operator(http_request),
        )
    except AirRateTableReviewNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except AirRateTableReviewTransitionError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except AirRateTableReviewConflictError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    except AirRateDocumentStorageError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    return {
...
@app.post("/air-rate-table-reviews/{review_id}/rows/{candidate_id}/decision")
def decide_air_rate_table_row_endpoint(
    review_id: str,
    candidate_id: str,
    request: AirRateTableRowDecisionRequest,
    http_request: Request,
):
    try:
        review = decide_air_rate_table_row(
            review_id=review_id,
            candidate_id=candidate_id,
            decision=request.decision,
            review_note=request.review_note,
...
@app.post("/air-rate-surcharge-reviews/{review_id}/candidates/{candidate_id}/operational-conditions")
def decide_air_rate_surcharge_operational_conditions_endpoint(
    review_id: str, candidate_id: str, request: AirRateSurchargeOperationalConditionsRequest, http_request: Request,
):
    try:
        review = decide_air_rate_surcharge_operational_conditions(
            review_id=review_id, candidate_id=candidate_id,
            cargo_applicability=request.cargo_applicability,
            routing_applicability=request.routing_applicability, via_airport=request.via_airport,
            review_note=request.review_note, reviewed_by=_authenticated_operator(http_request),
            repository=air_rate_surcharge_review_repository, source_repository=air_shadow_repository,
        )
    except AirRateSurchargeReviewNotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except AirRateSurchargeReviewTransitionError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    return {"review": review.model_dump(mode="json"), "runtime_authoritative": False, "pricing_authority_enabled": False, "calculation_consumption_enabled": False}


@app.post("/air-rate-surcharge-reviews/{review_id}/candidates/{candidate_id}/flat-quantity-basis")
def decide_air_rate_surcharge_flat_quantity_basis_endpoint(
    review_id: str, candidate_id: str, request: AirRateSurchargeFlatQuantityBasisRequest, http_request: Request,
):
    try:
        review = decide_air_rate_surcharge_flat_quantity_basis(
            review_id=review_id, candidate_id=candidate_id, flat_quantity_basis=request.flat_quantity_basis,
            review_note=request.review_note, reviewed_by=_authenticated_operator(http_request),
            repository=air_rate_surcharge_review_repository,
        )
    except AirRateSurchargeReviewNotFoundError as exc:
