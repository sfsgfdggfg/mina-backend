...
## RULE-163 — Extracted Attachment Content Must Be Privacy-Transformed and Cannot Change Workflow State

Only content that has already passed the attachment metadata, trusted-route, content-verification and safe-extraction boundaries may reach P1-57 interpretation. Email subject/body and extracted attachment sections are treated as untrusted source data, privacy-minimized independently, source-labeled with system-controlled labels and combined only within the 120,000-character interpretation limit. Input beyond that limit fails closed; truncation is not permitted.

Customer attachment interpretation may return a non-authoritative shipment proposal and supplier attachment interpretation may return a commercial-only extraction proposal. Neither result may be persisted or applied by P1-57. In particular, supplier attachment interpretation must not create a SupplierRFQResponse, advance an RFQ from `awaiting_response` or `clarification_required`, or create inbound-message ingestion evidence. Operator pull output may expose only safe interpretation status/reason/parser-call/privacy-version metadata; interpreted text, tables and structured parser payloads remain internal and excluded from serialization/repr surfaces.

Attachment AI interpretation must be opt-in for each controlled Outlook pull. Default pull behavior remains P1-56 extraction-only/manual-review. Deploying P1-57, restarting the pilot, or having a previously extracted attachment in the inbox is not authorization to send attachment-derived content to AI.

## RULE-164 — Interpreted Attachment Facts Need Explicit Review Apply Authority

A successful P1-57 interpretation may create a durable P1-58 review case, but it must not mutate customer or supplier workflow state until an authenticated operator explicitly applies that review. Operator list output may expose only review metadata; review detail may expose the structured interpreted candidate needed for human inspection, but must not expose attachment SHA-256 values, provider attachment IDs or raw/extracted source content.
...
## RULE-178 — Supplier Follow-Up Must Distinguish Silence, Acknowledgement, Commercial Result, and Secondary Release

For road RFQs, all selected primary suppliers must be handled as the first parallel dispatch group. Secondary suppliers must not be released merely because a primary supplier is slow, silent, unreachable by email, or because the customer says the price is urgent. A primary supplier is unavailable after an explicit terminal `no_capacity`/`declined` result; a quoted primary may also be treated as operationally unavailable for that shipment only when deterministic transit/availability evidence proves it cannot meet an explicit customer-required delivery date. Missing or uncertain timing evidence must be clarified and must not release secondary suppliers. A separate authenticated commercial-release path is required after completed primary price negotiation.

No-response timing begins at confirmed RFQ send. At 30 minutes with no acknowledgement or commercial result, a reminder is due. Continued silence after the reminder requires human telephone/WhatsApp escalation and may require manager/owner escalation; it does not authorize secondary dispatch. Acknowledgement means the supplier explicitly confirmed receipt/visibility or that it is working on the request. It is non-commercial evidence and starts a 120-minute road grace period. It must not be stored as `quoted`, `no_capacity`, `declined`, or `needs_clarification`.

Secondary commercial release after high primary pricing must fail closed unless every primary has a terminal result, at least one primary price quote exists, and the authenticated operator explicitly records `primary_price_negotiation_exhausted`. The authorization must not contain or expose the customer's target price. Existing RFQ approve/send guards remain authoritative, and secondary RFQ approval/send must recheck the current primary gate rather than trusting an earlier read.

The agency's default proactive customer deadline update lead is five minutes before an explicit quote-response deadline. Until a customer quote deadline is represented as validated structured data, the system must not infer an exact clock deadline or send a fabricated deadline update from unrelated delivery dates.

## RULE-179 — Automated Outbound Timing Must Respect Business-Time Clocks and Fail Closed Outside Hours

The default pilot business calendar is Europe/Istanbul, Monday-Friday, 09:00-18:30. Supplier no-response and acknowledgement timers count only minutes inside that calendar. Outside-hours intervals and weekends contribute zero elapsed reminder time. Automatic supplier reminders, automatic customer quote-deadline status updates and supplier phone/WhatsApp escalation work must remain paused outside the business window.

A silent supplier receives at most the approved reminder sequence: one reminder after 30 business minutes without acknowledgement/commercial response, or one reminder after 120 business minutes from explicit acknowledgement/working evidence. These are mutually state-driven; MINAI must not create a repeated periodic reminder loop. After a sent reminder, continued silence becomes human phone/WhatsApp escalation during business hours only.

A customer quote-response deadline must come from explicit customer timing evidence. Urgency words alone, required delivery dates and transit durations must never be converted into quote deadlines. When the explicit deadline is outside business hours, the one proactive status update should be moved to the last safe business send point, normally five minutes before business close. If that safe point is already missed, MINAI must not send a late after-hours substitute; later business-time handling is human attention.
...
## RULE-185 — Master Geography Must Be Explicit, Bounded, and Backward-Compatible

Supplier geography evidence must distinguish explicit country capability from explicit business-region capability. Country matching may normalize only known aliases such as `Almanya`/`Germany`/`DE`; it must not infer a country or region from vague text. A region capability is eligible for deterministic matching only when its country membership is explicitly stored with that region. Ambiguous legacy tags such as `western_europe` may be preserved as legacy metadata but must not be expanded into invented country membership.

Legacy supplier `priority_routes` may bootstrap stronger country evidence only when the destination country is explicitly present in the route string. In P2-03 bootstrap, an explicitly prioritized destination becomes `main_market`; other explicitly listed countries become `works`. Region strength, postcode-to-region mapping, historical-operation inference and email-derived geography remain outside this rule unless a future evidence-backed learning or geography decision explicitly authorizes them.

Customer names/aliases must remain globally collision-safe within the master repository. Active supplier contact email addresses must not be owned by two suppliers. Client-generated `entry_id` values make create/import retries idempotent; reusing the same identity with materially different master data must fail closed. A repeated legacy bootstrap must also fail closed when the previously imported legacy source has materially drifted, rather than silently leaving stale master data in place. Existing pilot customer/supplier JSON contracts remain supported through compatibility projection until a separate runtime cutover decision is accepted.

## RULE-186 — External Automation Must Obey the Most Specific Effective Policy at Planning and Send Time

For supplier reminders and customer deadline updates, MINAI must resolve the effective action mode using `Job → Customer → Agency → legacy dispatch fallback`. An explicit job mode wins over all broader policy. A legacy job disable remains a manual fail-safe when no modern job mode is set. Customer policy is used only from one active, deterministically matched customer master profile. Missing or ambiguous customer master evidence must not invent a customer-level override.

`manual` creates human work and must not be sent by the scheduler. `approval_required` creates a distinct review-and-approve work state and must not be sent by the scheduler. `automatic` permits scheduler delivery only after all existing timing, business-calendar, recipient, commercial-state and stale-action guards also pass. The effective policy must be re-evaluated immediately before provider delivery so a policy or operational-state change cannot authorize a stale automatic send.

Policy updates are partial. Omitted action fields preserve their prior value. Explicit `null` means inherit from the next broader policy layer. Existing legacy disable fields remain readable and writable for compatibility, and ordinary retention must not delete durable agency or customer automation-policy evidence.

## RULE-187 — Operational Milestones Must Be Evidence-Backed and Exceptions Must Not Pollute Stage Authority

For lifecycle-v2 jobs, controlled API stage transitions must fail closed when the milestone evidence required by that transition is missing. `vehicle_assigned` requires plate, driver name and assignment time; `loaded` requires `loaded_at`; `delivered` requires `delivered_at`; `closing_review` requires delivery evidence and at least one POD/CMR receipt timestamp. `completed` additionally requires that no operation exception remains open. Direct legacy service usage remains backward compatible when no operation-execution repository is supplied, and lifecycle-v1 jobs are not subjected to P2-05 evidence gates.
...
## RULE-281 — Preserve One Air Tariff Artifact Lineage and Reject Exact Semantic Double Counting End to End

Before consuming a commercial-air tariff calculation, require the confirmed table, completed structure review and completed surcharge review to refer to the same immutable source ID, source SHA and extracted-text lineage. Do not recover from mismatched review evidence by choosing a newer, nearest or same-named review. Source or review drift must block the cost calculation and therefore every downstream quote action.
...
## RULE-288 — GTIP / Commodity Conflict Is a Commercial Stop Until Current Facts Are Reconciled

When a shipment carries a GTIP/HS code that maps to an operational commodity incompatible with the current confirmed commodity, preserve the explicit commodity but fail closed before supplier RFQ or customer quote progression. The conflict must appear in operational consistency as both a visible verification warning and a blocking error.

Recompute the conflict from current shipment facts and the validated HS commodity map on every operational evaluation. Do not use the presence or absence of a `special_notes` warning string, or a legacy detection flag, as authority. After customer or customs-broker evidence resolves the GTIP or commodity value, the blocker may clear automatically if the current facts are compatible. MINAI must not silently rewrite the commodity or claim legally authoritative GTIP classification.
