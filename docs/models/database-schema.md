...
## P2-17 Air Freight Shadow Learning Foundation

`AirRateSource` is an immutable evidence record for `commercial_air` tariff PDFs. It stores `source_id`, idempotent `entry_id`, airline, PDF document name/MIME, SHA-256 fingerprint, cargo scope, optional origin-airport and validity evidence, authenticated recorder/time, bounded notes and `registered_not_interpreted` status. It intentionally stores no parsed tariff price or surcharge and is never runtime pricing authority.
...
### P2-31 Commercial-Air FX Rate Evidence
...
### P2-39 Commercial-Air Unsupported Cost Semantics Review and Cost Completeness Preview
...
### P2-44 Commercial-Air Outcome/Correction Evidence and Advisory Learning

P2-44 adds durable `air_learning_feedback` plus idempotency index `air_learning_feedback_by_entry`. Each immutable `air_learning_feedback_v1` record stores MINA job/code and P2-43 handoff identity; the complete frozen `air_quote_context_v1`; tariff-use result; actual airline/routing/via; optional actual service/delivery dates, chargeable weight and cost/currency; bounded correction categories; evidence source/reference/note; authenticated recorder/time; and optional `supersedes_feedback_id`. Old records are never overwritten. Only the single unsuperseded current record for a job participates in aggregation.
