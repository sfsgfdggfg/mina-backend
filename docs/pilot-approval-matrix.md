...
| `deployment_storage_approval` | Exact frozen release, external verified data pack, dedicated fresh pilot DB, authenticated HTTPS web access, approved host/storage permissions and shadow outbound mode. For the first-customer cloud pilot this includes the single-instance Railway service and `/data` persistent volume. | Technical/deployment owner | Exact-release deployment check (`cloud_pilot_launcher --check-only` for cloud) plus approval of the selected host/storage arrangement |
