...
## DEC-256 — Commercial-Air Cost Scope Must Be Classified Explicitly Before Any Completeness Claim

**Status:** Accepted
**Date:** 2026-09-14

P2-37 introduces inquiry-bound human review of the local-cost scope that may sit outside the reviewed airline tariff/surcharge chain. For each exact commercial-air source id/SHA and inquiry, the operator must classify every supported scope category exactly once: `pickup`, `origin_handling`, `origin_terminal`, `documentation`, `customs_service_fee`, `destination_handling`, `destination_terminal`, `delivery` and `other`. Each category is explicitly `required`, `not_applicable` or `unresolved` and carries an authored rationale; omission and duplicate-category substitution fail closed.
...
## DEC-258 — Commercial-Air Cost Completeness Requires Cleared Unsupported Semantics, Resolved Surcharge Cost and Reviewed Weight Rounding

**Status:** Accepted
**Date:** 2026-09-14

P2-39 may confirm a bounded inquiry-level air cost basis only by combining four explicit evidence gates: P2-38 required flat local-cost coverage must be complete; every unsupported additional-cost semantic (`weight_based_additional_cost`, `percentage_additional_cost`, `minimum_tiered_formula_additional_cost`, `customs_duties_taxes`, `other_unmodeled_cost`) must be reviewed for the exact source/inquiry and cleared as `not_applicable`; the reviewed airline surcharge preview must contain no cost-relevant unresolved exclusion; and exact-source airline weight rounding must have been explicitly reviewed, including explicit `none` when no rounding applies.
...
## DEC-261 — Durable Air Quotes Must Be MINA-Job-Bound and Freeze the Readiness Evidence Used for Human Approval

P2-42 may create a durable commercial-air QuoteCase only for an existing open `price_request` MINA job whose authoritative persisted shipment has just passed P2-41. The preparation request does not accept a second shipment payload; the backend reuses the MINA job shipment and reruns cost, pricing, operational, delivery-deadline and regulatory readiness before any write.

A successful preparation creates the existing shared QuoteCase + pending QuoteApproval lifecycle rather than a parallel air approval system. QuoteCase and QuoteApproval freeze the exact air evidence lineage used at preparation time: immutable tariff source/SHA, tariff row, cost-scope and unsupported-cost review IDs, validity/rounding/availability evidence, service/expected-delivery dates, confirmed cost basis, selected FX/local-cost evidence and pricing-policy source. Operator revisions create a fresh approval but preserve that same frozen air evidence snapshot.
...
## DEC-288 — Real Pilot Uses Fresh Isolated Persistence and Pre-Collected Approval Evidence
